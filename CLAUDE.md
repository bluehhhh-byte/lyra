...
- **진단·일괄 작업은 DB 전량 조회로 시작하지 마라**: 2026-09-12에 402가 재발했다. `songs/*.md` 백업은 DB와 같은 내용이고(`dump-content.mjs --check`가 대조한다) 파일 읽기는 전송량을 쓰지 않는다. 진단 스크립트의 기본 입력은 백업 파일이어야 하고, 백업에 아직 없는 최신 곡까지 봐야 할 때만 `--db`로 넘어간다(`scripts/audit-song-lang.mjs`, `scripts/audit-original-leak.mjs`가 그 꼴이다). **402 중에도 배포는 된다** — 2026-08-22 사고 기록의 "빌드가 sitemap에서 죽는다"는 지금 사실이 아니다. 그때 이후로 읽기 진입점 넷에 파일 폴백이 생겼고, sitemap 라우트 자체가 크롤 정책 변경 때 사라졌다. 2026-09-12에 `DATABASE_URL`을 못 쓰는 주소로 바꿔 빌드해 확인했다 — 폴백 100회, 626/626 페이지, exit 0. 막히는 것은 **쓰기**다(관리자 저장·곡 등록). 전송량은 Neon 콘솔 API로만 읽을 수 있고(`scripts/quota-watch.mjs`), 그러려면 `NEON_API_KEY`·`NEON_PROJECT_ID`가 있어야 한다. 지금 `.env.local`에는 없다.
- **Gemini 모델 변덕**: 하루 안에 latest 503→200, 3.6-flash 200→무응답으로 뒤집혔다. 특정 버전을 기본으로 박지 마라. 별칭 + `GEMINI_MODEL_FALLBACKS`가 답이다.
- **Google 검색 그라운딩은 무료 티어에 없다 — 기다려도 안 풀린다**: 2026-09-05부터 `google_search`를 켠 요청이 전부 429다. 한도 소진이 아니라 요금제 차단이다. 2026-09-12 실측 넷: ①그라운딩 켠 요청만 429, 끈 요청은 같은 모델에서 200 ②429 본문에 `QuotaFailure`도 `retryDelay`도 없다(속도 제한이면 둘 다 온다) ③공식 가격표상 무료 티어 Gemini 3.x는 Search 그라운딩 "Not available" ④그라운딩이 무료인 2.5 Flash/Flash-Lite(500 RPD)는 이 키에서 그라운딩과 무관하게 404 — "no longer available to new users". **재시도를 권하는 문구를 쓰지 마라** — 예전 "한도가 초기화된 뒤 다시 시도하세요"가 사용자를 무한 재시도로 몰았다. 지금은 `GROUNDING_UNAVAILABLE` 한 문구를 쓰고, 한 번 429를 보면 한 시간 동안 네트워크를 두드리지 않는다(`groundingBlocked`). 대체 경로는 `lib/admin/appearance-search.js`다 — Apple Music(OST 앨범명)과 위키백과(곡 문서의 타이업 문장)로 후보를 찾고, **일반**(그라운딩 아닌) Gemini가 그중에서 고른다. 근거 URL은 늘 API 응답에서 오므로 모델이 지어낼 수 없다. 검증된 79건 기준 재현율 22%(한국 46%·영어 18%·일본 14%)라 그라운딩에 못 미치므로, **못 찾은 것을 '수록 정보 없음'으로 내보내지 않는다**(곡의 92%는 실제로 수록이 없어서, 빈손을 확정으로 읽으면 틀릴 때가 훨씬 많다). `BRAVE_API_KEY`(무료 월 2,000건)를 넣으면 일반 웹 검색까지 더해진다 — 곡당 1회만 부른다. 기존 960곡은 2026-09-05에 조사가 끝나 있다.
