...
13. 리디자인 1차 "리스닝 룸 × 아카이브" (A+B 하이브리드):
    - **앨범색 추출** (`lib/palette.js`) — canvas 4-bit 버킷, localStorage 캐시,
      CORS 실패 시 기본 라벤더 팔레트로 저하
    - **앰비언트 라이트** (`app/ambient.js`) — 곡 페이지는 페이지 전면(fixed),
      홈 캐러셀은 섹션 스코프(absolute). `@property`로 팔레트 전환 크로스페이드
    - **홈 캐러셀** (`app/carousel.js`) — 최근 8곡 스냅 트랙, 포커스 카드가
      섹션 조명을 바꿈. 검색·필터 중엔 숨김
    - **가사 리딩 모드** — 원문 확대 + 번역은 호버/탭/포커스 시 잉크 리빌
14. 인터랙션 레이어 (`globals.css` 하단) — 라이브러리 없이 CSS 중심:
