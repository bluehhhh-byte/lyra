...
- **기록의 흐름** (`/songs/thread`): 최근 기록 20곡을 담은 순서 그대로 놓고, '모든'
  인접한 기록 사이를 가사의 맥락으로 잇는 끊어지지 않는 타임라인 + 전체가 그리는
  하나의 이야기(story). admin '흐름 분석' 버튼 → data/thread.json.
  (발견 경로(다리·주제 코스)는 만들었다가 제거 — 흐름 추적이 Lyra의 성격에 맞았다)
- **Lyra×Syno 교차**: 영화 상세에 '이 시대의 음악'(같은 연대 컬렉션 곡, 같은 권역 우선),
