...
- **발견 경로** (`/songs/paths`): 두 곡 사이의 다리(bridge) 또는 주제 코스(theme) —
  시작·전환·도착 구조의 6~10곡, 정거장마다 이유와 30초 미리듣기. 컬렉션 곡은 slug로
  연결, 새 곡은 iTunes 매칭. 다리는 두 곡의 가사 모티프를 재료로 쓴다.
  admin '발견 경로' 패널 → data/paths.json (최신 30개)
