...
- **영화 탐색** (`/movies`): 제목·감독·배우·줄거리 검색, 영화/드라마·국가·장르·최소 별점 필터,
  기록일·개봉연도·별점·제목 정렬과 랜덤 보기. 모든 조건은 URL에 저장된다
- **큐레이션 컬렉션** (`/collections`): 작품을 원하는 순서로 묶어 소개문과 함께 공개.
  admin에서 검색·추가·순서 변경·공개/비공개·삭제를 관리하며 `collections/*.md`에 저장한다
- **문화 아카이브** (`/archive`): 같은 날 기록한 음악·영화를 월별 타임라인과 날짜 상세로 통합
- **워치리스트** (`/watchlist`): 보고 싶음·보는 중·완료·중단 상태, OTT, 회차, 시작·완료일 관리
- **통합 검색**: 헤더에서 음악·가사·영화·줄거리·컬렉션·감독·배우를 한 번에 검색
- **문화 결산** (`/recap`): 월간·연간 기록량·감정·키워드·가수·최고 별점 작품과 공유 카드
- **인물 페이지** (`/people`): 영화 파일의 감독·출연진을 역색인해 참여 작품과 평균 별점 표시
- **통계** (`/stats`): 국가·연대·가수·장르·태그 분포, 기록 월·시간대,
  영화·드라마 비율·월별 감상·개봉 연대·국가·장르·감독·별점·러닝타임,
  **감정 변화 시계열**(날짜별 감정을 밝음↔어두움 valence로 그린 SVG 곡선)
- **키워드 일기** (`/diary`): 월 이동·월간 요약·감정 달력·선택 날짜 상세·접힌 월간 기록.
