...
- **곡 페이지**: 원문(세리프)·독음·번역 대역 표시, **읽기 모드 토글**(원문만/번역만/둘 다),
  앨범아트 히어로, iTunes 미리듣기 + YouTube 링크, 곡 코멘트, 연 단위 해설 노트,
  가수·앨범·연도 표기(일본 아티스트는 한글 독음 병기), **관련 곡 추천**(가수·태그 기반)
- OG 메타태그 — 공유 시 앨범아트 카드

### 관리자 (`/admin`, 비밀번호 보호)
- **곡 추가**: 언어 선택 → iTunes 검색(미·한·일 스토어 통합, 관련도 정렬, 더 보기 페이징)
