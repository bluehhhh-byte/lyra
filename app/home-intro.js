...
export default function HomeIntro({ insights }) {
  ...
  return (
    <div className="relative isolate mb-16 overflow-hidden pt-4">
      <section
        className={`mb-12 min-w-0 overflow-hidden border-y border-line bg-bg/[0.88] ${latest?.text ? "sm:grid sm:grid-cols-[minmax(0,1fr)_17rem]" : ""}`}
        aria-labelledby="cultural-biography-title"
      >
        <div className="min-w-0">
          ...
          <h1 id="cultural-biography-title" className="max-w-4xl text-balance font-serif text-3xl leading-tight sm:text-5xl sm:leading-tight">
          ...
          <LatestDayText latest={latest} />
        </div>

        {latest?.text && (
          <figure className="flex min-w-0 flex-col border-t border-line bg-surface/55 sm:border-l sm:border-t-0">
