...
${lines}`
  ...
  if (action === "songThread") {
    ...
    // 감정 라벨은 코퍼스에서 뺀다 — 넣으면 연결이 전부 '불안이 고독으로…'식
    // 감정 서사로 흘러버린다. 가사 줄을 6줄로 늘려 '내용'이 재료가 되게 한다.
    const corpus = recent
      .map(
        (s, i) =>
          `${i + 1}. [${s.slug}] ${s.title} - ${s.artist}\n${koOf(s).slice(0, 6).join(" / ").slice(0, 360)}`
      )
      .join("\n");

    const raw = await geminiText(
      key,
      `아래는 한 사람이 최근에 '기록한 순서대로' 나열한 ${recent.length}곡의 가사다 (번호가 시간 순서).
가사의 '내용' — 장면, 사물, 장소, 인물, 행위 — 이 어떻게 이어지는지로 하나의 이야기를 읽어라. JSON으로만:
{"story":"가사 속 장면들이 이어져 그리는 하나의 이야기 3~4문장 — 반드시 평서문 '~다'체, ~습니다/~해요 금지","connections":["1번 곡의 가사 내용이 2번 곡의 가사 내용으로 이어지는 한 구절 (45자 이내)", "... 총 ${recent.length - 1}개, i번째 항목이 i번 곡→i+1번 곡의 연결"]}
규칙:
- connections는 반드시 ${recent.length - 1}개 — 모든 인접한 기록 쌍을 빠짐없이 잇는다
- 감정 단어(불안·고독·저항·분노·슬픔 등)로 잇지 말 것. 두 곡의 가사에 실제로 나오는
  구체적 이미지·장면·사물·행위를 붙여라 (예: "뒤집힌 방의 가구들이 다음 곡에선 남의 의자를 옮기는 장난이 된다")
- story도 감정 흐름이 아니라 가사 속 장면들이 이어지는 서사로
- 억지로 갖다 붙이지 말고, 정말 접점이 없으면 장면이 어떻게 바뀌는지(전환)를 서술하라
- 순수 JSON만 출력
기록:
${corpus}`,
      true
