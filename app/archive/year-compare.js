    ...
    <>
      <div className="mb-5 flex flex-wrap items-end gap-3 rounded-xl border border-line bg-surface p-4">
        <label className="grid gap-1 text-xs text-muted">
          첫 번째 연도
          <select value={firstYear} onChange={(event) => setFirstYear(event.target.value)} className="rounded-lg border border-line bg-bg px-3 py-2 text-sm text-ink">
            {years.map((value) => <option key={value} value={value}>{value}년</option>)}
          </select>
        </label>
        <label className="grid gap-1 text-xs text-muted">
          두 번째 연도
          <select value={secondYear} onChange={(event) => setSecondYear(event.target.value)} className="rounded-lg border border-line bg-bg px-3 py-2 text-sm text-ink">
            {years.map((value) => <option key={value} value={value}>{value}년</option>)}
          </select>
        </label>
      </div>
