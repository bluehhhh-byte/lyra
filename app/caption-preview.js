...
export default function InstagramCaptionPreview({ text, className = "bg-surface" }) {
  ...
  return (
    <div>
      <pre className={`max-w-full whitespace-pre-wrap break-words  border border-line px-3 py-2 font-sans text-xs leading-relaxed text-ink ${className}`}>{preview.text}</pre>
