  ...
  return (
    <>
      <div className="mb-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
        <input value={q} onChange={(event) => setQ(event.target.value)} placeholder="제목·감독·배우 검색" aria-label="평가한 영화 검색" className=" border border-line bg-surface px-3 py-2 text-sm outline-none focus:border-accent lg:col-span-2" />
        ...
        {Array.from({ length: 10 }, (_, index) => +(5 - index * 0.5).toFixed(1)).map((score) => {
          ...
          return (
            <button key={score} onClick={() => setRating(active ? "" : String(score))} disabled={!count} aria-pressed={active} className="flex flex-1 flex-col items-center gap-1 disabled:cursor-default" title={count ? `★${score} ${count}편` : `★${score} 없음`}>
              ...
              <span className={`w-full  transition-colors ${active ? "bg-accent" : count ? "bg-accent/50 hover:bg-accent/80" : "bg-line"}`} style={{ height: `${Math.max(2, (count / max) * 80)}px` }} />
      ...
      {visible < shown.length && (
        <div className="mt-10 flex justify-center">
          <button onClick={() => setVisible((count) => count + STEP)} className=" border border-line px-5 py-2 text-sm text-muted hover:border-accent hover:text-accent">나머지 {(shown.length - visible).toLocaleString("ko-KR")}편 더 보기</button>
        </div>
      )}
    </>
  );
...
function Select({ label, value, onChange, options }) {
  return (
    <label className="sr-only">
      {label}
      <select value={value} onChange={(event) => onChange(event.target.value)} className="not-sr-only w-full  border border-line bg-surface px-3 py-2 text-sm outline-none focus:border-accent">
  ...
  const inner = (
    <>
      <div className="relative overflow-hidden  border border-line bg-surface">
        ...
        <span className="absolute right-1 top-1  bg-black/70 px-1.5 py-0.5 text-xs font-semibold text-white tabular-nums">★{movie.rating}</span>
        {movie.internalHref && <span className="absolute bottom-1 left-1  bg-accent px-1.5 py-0.5 text-[10px] font-semibold text-bg">Cyno 기록</span>}
      </div>
