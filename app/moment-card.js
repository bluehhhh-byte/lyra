...
export default function MomentCard({ moment, compact = false }) {
  ...
  return (
    <Link href={`/moments/${encodeURIComponent(moment.slug)}`} className="group block rounded-xl border border-line bg-surface/40 p-5 transition hover:border-accent/60 hover:bg-surface">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-[11px] text-muted">
        ...
        {moment.emotions.slice(0, 3).map((emotion) => <span key={emotion} className="rounded-full bg-accent/10 px-2 py-0.5 text-accent">{emotion}</span>)}
      </div>
    </Link>
  );
}
