    ...
    <div>
      {saved > 0 && (
        <div className="mb-3 flex justify-end">
          <button onClick={() => setSavedOnly((value) => !value)} aria-pressed={savedOnly} className={`rounded-full border px-3 py-1 text-xs ${savedOnly ? "border-accent bg-accent text-bg" : "border-line text-muted hover:text-accent"}`}>
      ...
      {visible.length ? (
            ...
            return (
              <article key={id} className="group">
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  <button onClick={() => update(id, "saved")} aria-pressed={isSaved} className={`rounded-full border px-2 py-1 text-[10px] ${isSaved ? "border-accent bg-accent text-bg" : "border-line text-muted hover:text-accent"}`}>보고 싶음</button>
                  <button onClick={() => update(id, "seen")} className="rounded-full border border-line px-2 py-1 text-[10px] text-muted hover:text-accent">이미 봄</button>
                  <button onClick={() => update(id, "dismissed")} className="rounded-full border border-line px-2 py-1 text-[10px] text-muted hover:text-accent">관심 없음</button>
                </div>
              </article>
            );
          })}
        ...
        <p className="rounded-xl border border-dashed border-line px-4 py-10 text-center text-sm text-muted">{savedOnly ? "저장한 추천이 없습니다." : "이 회차의 추천을 모두 분류했습니다."}</p>
      )}
    </div>
  );
}
