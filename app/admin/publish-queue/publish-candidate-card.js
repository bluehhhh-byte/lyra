...
export default function PublishCandidateCard({ item }) {
  ...
  if (done) return <li role="status" className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-emerald-700 dark:text-emerald-300">{item.title} · 발행 완료</li>;
  return (
    <li className="rounded-2xl border border-line bg-surface p-3 transition hover:border-accent/60">
      ...
      <button type="button" onClick={complete} disabled={busy} className="mt-3 w-full rounded-lg border border-line px-3 py-2 text-xs font-semibold text-muted hover:border-emerald-500 hover:text-emerald-600 disabled:opacity-50">
        {busy ? "저장 중…" : "발행 완료"}
      </button>
