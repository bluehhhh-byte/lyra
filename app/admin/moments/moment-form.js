    ...
    <form onSubmit={submit} className="max-w-2xl space-y-6">
      ...
      <fieldset className="rounded-xl border border-line p-4">
        ...
        <div className="mb-3 flex gap-2">
          {[["song", "노래"], ["movie", "영화"]].map(([value, label]) => <button key={value} type="button" onClick={() => { setKind(value); setQuery(""); }} className={`rounded-full px-3 py-1.5 text-xs ${kind === value ? "bg-accent text-white" : "bg-surface text-muted"}`}>{label}</button>)}
        </div>
        <input className={input} value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`${kind === "song" ? "곡·가수" : "영화·감독"} 검색`} />
        {results.length > 0 && <div className="mt-2 overflow-hidden rounded-lg border border-line">{results.map((item) => <button key={`${item.kind}:${item.slug}`} type="button" onClick={() => add(item)} className="flex w-full items-center justify-between border-b border-line px-3 py-3 text-left text-sm last:border-0 hover:bg-surface"><span>{item.title}</span><span className="ml-3 truncate text-xs text-muted">{item.subtitle} ＋</span></button>)}</div>}

        <div className="mt-5 space-y-4">
          {form.links.map((link, index) => {
            const item = catalogMap.get(`${link.targetKind}:${link.targetSlug}`);
            return <div key={`${link.targetKind}:${link.targetSlug}`} className="rounded-lg bg-surface p-3">
