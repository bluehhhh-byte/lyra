...
async function drawSingle(slide, images, position, carousel) {
  ...
  if (slide.role === "cover") {
    ...
    points.slice(0, 3).forEach((point, index) => {
      const y = 350 + index * 235;
      ctx.fillStyle = "rgba(13,13,15,0.76)";
      ctx.beginPath(); ctx.roundRect(PAD, y - 54, W - PAD * 2, 188, 24); ctx.fill();
      ctx.fillStyle = ACCENT;
      ctx.font = `800 30px ${SANS}`;
      ctx.fillText(String(index + 1).padStart(2, "0"), PAD + 34, y);
      ctx.fillStyle = INK;
      drawFittedParagraph(ctx, point, PAD + 112, y, W - PAD * 2 - 160, 110, 37, 27);
    });
    if (isViewing && slide.note) {
      ctx.fillStyle = DIM;
      ctx.font = `500 25px ${SANS}`;
      const note = wrap(ctx, `“${slide.note}”`, W - PAD * 2).slice(0, 3);
      note.forEach((line, index) => ctx.fillText(line, PAD, 1120 + index * 36));
    }
  }
