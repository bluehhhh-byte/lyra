...
import { monthlyStats, yearCalendar, statYears, compareYearStats, monthSeasonality, monthNarrative, releaseRecordGaps, yearNarrative, workLabel, TURNING_POINT_DISTANCE } from "./archive-stats.js";
...
// 월 집계와 좌표
{
  ...
  assert.equal(mar.character, "조용한 기록의 달");
...
// 전환점 — 연속된 월의 큰 이동만 표시하고 빈 달을 건너뛴 이동은 제외한다
{
  const turningStats = monthlyStats(buildArchive({
    songs: [
      song("t1", "2025-01-01", "슬픔"), song("t2", "2025-01-02", "슬픔"), song("t3", "2025-01-03", "고독"),
      song("t4", "2025-02-01", "기쁨"), song("t5", "2025-02-02", "기쁨"), song("t6", "2025-02-03", "설렘"),
      song("t7", "2025-04-01", "슬픔"), song("t8", "2025-04-02", "슬픔"), song("t9", "2025-04-03", "고독"),
    ], movies: [],
  }));
  assert.ok(turningStats[1].prev.distance >= TURNING_POINT_DISTANCE);
  assert.equal(turningStats[1].turningPoint, true);
  assert.equal(turningStats[2].prev.gap, 1);
  assert.equal(turningStats[2].turningPoint, false, "빈 달을 건너뛴 이동은 월 대 월 전환점이 아니다");
}
console.log("✓ 정서 전환점 거리 임계값·연속 월 제한");
...
// 계절성 — 같은 달이 서로 다른 3개 연도에 있어야 해석한다
{
  const seasonal = monthSeasonality([
    { month: "2022-09", center: { v: 1, a: 1, n: 2 } },
    { month: "2023-09", center: { v: 2, a: 1, n: 1 } },
    { month: "2024-09", center: { v: 1, a: 2, n: 3 } },
    { month: "2022-10", center: { v: -1, a: -1, n: 5 } },
    { month: "2023-10", center: { v: -1, a: -1, n: 5 } },
  ]);
  assert.equal(seasonal.length, 12);
  assert.equal(seasonal[8].deferred, false);
  assert.equal(seasonal[8].years.length, 3);
  assert.match(seasonal[8].summary, /기록은/);
  assert.equal(seasonal[9].deferred, true);
  assert.equal(seasonal[9].type, "판단 유보");
}
console.log("✓ 같은 달 계절성·3개 연도 미만 유보");
...
// 표본이 적은 달은 단정하지 않는다
{
  ...
  assert.equal(sparse[0].character, "", "3곡 미만이면 달 성격도 표시하지 않는다");
