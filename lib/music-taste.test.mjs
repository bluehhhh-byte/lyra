...
// 최근 vs 전체 — 앞쪽(최신) n곡의 이동을 잡아야 한다
{
  const older = Array.from({ length: 10 }, (_, i) => song({ artist: `old${i}`, tags: ["영미", "Indie Rock", "2005"], emotion: "고독" }));
  const newer = Array.from({ length: 5 }, (_, i) => song({ artist: `new${i}`, tags: ["일본", "J-Rock", "2020"], year: "2020", emotion: "기쁨" }));
  const shifted = recentShift([...newer, ...older], 5); // 최신순 배열
  assert.equal(shifted.genre.name, "J-Rock", "최근 최다 장르");
  assert.ok(shifted.genre.deltaPct > 0, "전체 대비 상승");
  assert.equal(shifted.newArtists.length, 5, "처음 등장한 아티스트");
  assert.ok(shifted.valenceRecent > shifted.valenceAll, "최근이 더 밝음");
  assert.equal(recentShift(older.slice(0, 8), 10), null, "표본 부족이면 null");
}
console.log("✓ 최근의 변화");
