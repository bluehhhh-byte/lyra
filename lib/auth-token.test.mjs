...
// HMAC 마지막 글자가 우연히 0이면 고정 문자열 "0"은 변조가 아니다. 항상
// 원문과 다른 글자를 골라 확률적으로 실패하는 인증 회귀 테스트를 막는다.
const changedLast = t.endsWith("0") ? "1" : "0";
assert.equal(await verifyToken("secret", t.slice(0, -1) + changedLast), false, "서명 변조");
const [exp] = t.split(".");
