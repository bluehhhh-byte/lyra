import fs from "fs";
import path from "path";
import { readRuntimeData } from "./store.js";
import { runtimeBucket } from "./taste-core.js";

// 왓챠에서 평가한 영화 ~1,000편의 데이터셋. 개별 .md 페이지 대신 한 파일에
// 모아 둔다(data/watcha-movies.json) — 취향 분석·별점 목록의 원천.
// 별점은 왓챠 재추출본을 code로 병합한다(data/watcha-ratings.json이 있으면).
//
// 스키마(항목): { code, title, title_ko, media, year, runtime, director,
//   director_ko, cast[], genre, country, poster, tmdbId, rating|null, isReview }

const DATA = path.join(process.cwd(), "data", "watcha-movies.json");
const RATINGS = path.join(process.cwd(), "data", "watcha-ratings.json");

function readJson(p, fallback) {
  try {
    return JSON.parse(fs.readFileSync(p, "utf8"));
  } catch {
    return fallback;
  }
}

let CACHE = null;

export function getWatched() {
  if (CACHE) return CACHE;
  const watched = readWatched();
  if (process.env.NODE_ENV === "production") CACHE = watched;
  return watched;
}

// 왓챠 1045편 — 별점 병합까지 매 호출마다 다시 하지 않도록 위에서 캐시한다
function readWatched() {
  const movies = readJson(DATA, []);
  // { code: rating } — 관리자 임포트가 써두면 여기서 별점을 채운다
  const ratings = readJson(RATINGS, {});
  return movies.map((m) => ({
    ...m,
    // 파일 자체 rating이 있으면 우선, 없으면 재추출본에서
    rating: m.rating ?? (ratings[m.code] != null ? Number(ratings[m.code]) : null),
  }));
}

// 별점 있는 것만 (취향 "호불호" 분석은 평가된 것만 의미가 있다)
export const getRated = () => getWatched().filter((m) => m.rating != null);

export async function getWatchedRuntime() {
  const [movies, ratings] = await Promise.all([
    readRuntimeData("watcha-movies.json", []),
    readRuntimeData("watcha-ratings.json", {}),
  ]);
  return movies.map((movie) => ({
    ...movie,
    rating: movie.rating ?? (ratings[movie.code] != null ? Number(ratings[movie.code]) : null),
  }));
}

const numericRating = (movie) => {
  const rating = Number(movie?.rating);
  return Number.isFinite(rating) && rating >= 0.5 && rating <= 5 ? rating : null;
};

// 고정된 반 칸 버킷을 모두 돌려준다. 0건인 별점도 화면에 남아 분포 축이
// 데이터에 따라 흔들리지 않는다.
export function ratingDistribution(movies) {
  const counts = new Map(Array.from({ length: 10 }, (_, index) => [(index + 1) / 2, 0]));
  let total = 0;
  for (const movie of movies) {
    const rating = numericRating(movie);
    if (rating == null || !counts.has(rating)) continue;
    counts.set(rating, counts.get(rating) + 1);
    total += 1;
  }
  return [...counts].map(([rating, count]) => ({
    rating,
    count,
    share: total ? count / total : 0,
  }));
}

// Watcha 데이터에는 관람일이 없으므로 여기서 말하는 연도는 작품 공개연도다.
// 숫자인 연도만 사용하고 오름차순으로 고정해 같은 입력은 같은 선을 만든다.
export function yearlyRatingTrend(movies) {
  const years = new Map();
  for (const movie of movies) {
    const rating = numericRating(movie);
    const year = Number(movie?.year);
    if (rating == null || !Number.isInteger(year) || year < 1888 || year > 2100) continue;
    const row = years.get(year) || { year, count: 0, sum: 0 };
    row.count += 1;
    row.sum += rating;
    years.set(year, row);
  }
  return [...years.values()]
    .sort((a, b) => a.year - b.year)
    .map(({ year, count, sum }) => ({ year, count, average: sum / count }));
}

function ratingGroups(movies, keyFn) {
  const groups = new Map();
  for (const movie of movies) {
    const rating = numericRating(movie);
    if (rating == null) continue;
    const keys = Array.isArray(keyFn(movie)) ? keyFn(movie) : [keyFn(movie)];
    for (const value of keys) {
      const key = String(value || "").trim();
      if (!key) continue;
      const row = groups.get(key) || { k: key, n: 0, sum: 0 };
      row.n += 1;
      row.sum += rating;
      groups.set(key, row);
    }
  }
  return [...groups.values()].map(({ k, n, sum }) => ({ k, n, avg: sum / n }));
}

export function directorPreferences(movies, { min = 3, limit = 10 } = {}) {
  const rated = movies.filter((movie) => numericRating(movie) != null);
  const mean = rated.length
    ? rated.reduce((sum, movie) => sum + numericRating(movie), 0) / rated.length
    : 0;
  const rows = ratingGroups(rated, (movie) => movie.director_ko || movie.director);
  const eligible = rows.filter((row) => row.n >= min);
  return {
    mean,
    min,
    excluded: rows.length - eligible.length,
    byCount: [...rows].sort((a, b) => b.n - a.n || a.k.localeCompare(b.k, "ko")).slice(0, limit),
    high: eligible
      .filter((row) => row.avg > mean)
      .sort((a, b) => b.avg - a.avg || b.n - a.n || a.k.localeCompare(b.k, "ko"))
      .slice(0, limit),
    low: eligible
      .filter((row) => row.avg < mean)
      .sort((a, b) => a.avg - b.avg || b.n - a.n || a.k.localeCompare(b.k, "ko"))
      .slice(0, limit),
  };
}

export function frequentActors(movies, { min = 3, limit = 50 } = {}) {
  return ratingGroups(movies, (movie) => movie.cast)
    .filter((row) => row.n >= min)
    .sort((a, b) => b.n - a.n || b.avg - a.avg || a.k.localeCompare(b.k, "ko"))
    .slice(0, limit);
}

const normalizedMovieTitle = (movie) => String(movie?.title_ko || movie?.title || "")
  .normalize("NFKC")
  .trim()
  .toLocaleLowerCase("ko-KR");

export function rewatchGroups(movies) {
  const groups = new Map();
  for (const movie of movies) {
    const title = normalizedMovieTitle(movie);
    if (!title) continue;
    const key = movie.tmdbId != null && String(movie.tmdbId).trim()
      ? `tmdb:${movie.tmdbId}`
      : `title:${title}:${movie.year || ""}`;
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(movie);
  }
  return [...groups.entries()]
    .filter(([, entries]) => entries.length > 1)
    .map(([key, entries]) => {
      const ratings = entries.map(numericRating).filter((rating) => rating != null);
      return {
        key,
        title: entries[0].title_ko || entries[0].title,
        year: entries[0].year || "",
        count: entries.length,
        ratings,
        ratingChanged: new Set(ratings).size > 1,
      };
    })
    .sort((a, b) => b.count - a.count || a.title.localeCompare(b.title, "ko"));
}

export function directorOtherWorks(movies, current, { limit = 8 } = {}) {
  const director = String(current?.director_ko || current?.director || "").trim();
  if (!director) return [];
  const currentTmdb = current?.tmdbId == null ? "" : String(current.tmdbId);
  const currentTitle = normalizedMovieTitle(current);
  return movies
    .filter((movie) => String(movie.director_ko || movie.director || "").trim() === director)
    .filter((movie) => {
      if (currentTmdb && movie.tmdbId != null) return String(movie.tmdbId) !== currentTmdb;
      return normalizedMovieTitle(movie) !== currentTitle || String(movie.year || "") !== String(current.year || "");
    })
    .sort((a, b) => Number(b.rating ?? -1) - Number(a.rating ?? -1)
      || Number(b.year || 0) - Number(a.year || 0)
      || normalizedMovieTitle(a).localeCompare(normalizedMovieTitle(b), "ko"))
    .slice(0, limit)
    .map((movie) => ({
      key: movie.code || (movie.tmdbId ? `tmdb-${movie.tmdbId}` : `${normalizedMovieTitle(movie)}-${movie.year || ""}`),
      title: movie.title_ko || movie.title,
      year: movie.year || "",
      rating: numericRating(movie),
      poster: movie.poster || "",
      tmdbId: movie.tmdbId || null,
      media: movie.media || "movie",
    }));
}

const watchedDate = (movie) => movie?.watchedAt || movie?.watched_at || movie?.recordedAt || movie?.date || "";

export function watchedTimeline(movies) {
  const releaseYears = new Map();
  const viewingYears = new Map();
  const viewingMonths = new Map();
  let missingDates = 0;

  for (const movie of movies) {
    const releaseYear = Number(movie?.year);
    if (Number.isInteger(releaseYear) && releaseYear >= 1888 && releaseYear <= 2100) {
      releaseYears.set(releaseYear, (releaseYears.get(releaseYear) || 0) + 1);
    }

    const day = String(watchedDate(movie)).slice(0, 10);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(day) || Number.isNaN(Date.parse(`${day}T00:00:00Z`))) {
      missingDates += 1;
      continue;
    }
    const year = day.slice(0, 4);
    const month = day.slice(0, 7);
    viewingYears.set(year, (viewingYears.get(year) || 0) + 1);
    viewingMonths.set(month, (viewingMonths.get(month) || 0) + 1);
  }

  const rows = (map) => [...map].sort(([a], [b]) => String(a).localeCompare(String(b))).map(([period, count]) => ({ period, count }));
  return {
    total: movies.length,
    missingDates,
    releaseYears: rows(releaseYears),
    viewingYears: rows(viewingYears),
    viewingMonths: rows(viewingMonths),
  };
}

export function genreRatingCross(movies, { min = 3 } = {}) {
  const rows = ratingGroups(movies, (movie) => movie.genre || "미분류")
    .map((row) => ({ ...row, deferred: row.n < min }))
    .sort((a, b) => b.n - a.n || b.avg - a.avg || a.k.localeCompare(b.k, "ko"));
  return {
    min,
    maxCount: Math.max(...rows.map((row) => row.n), 1),
    deferred: rows.filter((row) => row.deferred).length,
    rows,
  };
}

export function countryDistribution(movies) {
  const counts = new Map();
  let unclassified = 0;
  for (const movie of movies) {
    const raw = String(movie?.country || "").trim();
    const country = raw || "미분류";
    if (!raw) unclassified += 1;
    counts.set(country, (counts.get(country) || 0) + 1);
  }
  return {
    total: movies.length,
    unclassified,
    rows: [...counts].map(([country, count]) => ({ country, count, share: movies.length ? count / movies.length : 0 }))
      .sort((a, b) => b.count - a.count || a.country.localeCompare(b.country, "ko")),
  };
}

const RUNTIME_BUCKETS = ["~90분", "90–110분", "110–130분", "130–150분", "150분+"];

export function runtimeInsights(movies, { longMinutes = 150 } = {}) {
  const bucketCounts = new Map(RUNTIME_BUCKETS.map((bucket) => [bucket, 0]));
  const periods = new Map();
  let missing = 0;
  let known = 0;
  let longCount = 0;

  for (const movie of movies) {
    const runtime = Number(movie?.runtime);
    if (!Number.isFinite(runtime) || runtime <= 0) {
      missing += 1;
      continue;
    }
    known += 1;
    const bucket = runtimeBucket(runtime);
    bucketCounts.set(bucket, (bucketCounts.get(bucket) || 0) + 1);
    const isLong = runtime >= longMinutes;
    if (isLong) longCount += 1;

    const year = Number(movie?.year);
    if (!Number.isInteger(year) || year < 1888 || year > 2100) continue;
    const start = Math.floor(year / 5) * 5;
    const row = periods.get(start) || { start, end: start + 4, known: 0, long: 0, runtimeSum: 0 };
    row.known += 1;
    row.runtimeSum += runtime;
    if (isLong) {
      row.long += 1;
    }
    periods.set(start, row);
  }

  return {
    longMinutes,
    known,
    missing,
    longCount,
    buckets: RUNTIME_BUCKETS.map((bucket) => ({ bucket, count: bucketCounts.get(bucket) })),
    periods: [...periods.values()].sort((a, b) => a.start - b.start).map((row) => ({
      ...row,
      average: row.runtimeSum / row.known,
      longShare: row.long / row.known,
    })),
  };
}
