...
test("홈 아트워크는 오디오와 분리된 서버 렌더링 SVG다", async () => {
  const [component, home, page, player] = await Promise.all([
    readFile(new URL("../app/report-artwork.js", import.meta.url), "utf8"),
  ...
  assert.ok(!/["']use client["']/.test(component));
  assert.ok(!/canvas|requestAnimationFrame|AnalyserNode|audioRef/.test(component));
  assert.match(home, /<ReportArtwork/);
