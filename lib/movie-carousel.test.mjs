...

{
  ...
  // 서사 포인트는 줄거리 문장에서만 온다 — 필러로 3개를 채우던 규칙은 죽었다.
  // 줄거리가 두 문장이면 포인트는 하나다(첫 문장은 4장 하단 요약이 가져간다).
  assert.ok(draft.keyPoints.length >= 1);
  assert.ok(!draft.keyPoints.some((point) => /이라는 주제가/.test(point)), "상투 필러가 서사 자리에 오면 안 된다");
  // 감상 포인트는 장르·국가·연도의 맥락이다 — 감독·배우 나열이 아니다.
  assert.ok(draft.viewingPoints.some((point) => /2010년대|Comedy/.test(point)));
  // 카드별 하단 인용문 — 각 카드 요약이고 서로 달라야 하며 2장 소개문 재탕이 아니다.
  assert.ok(draft.keyPointsNote);
  assert.ok(draft.viewingPointsNote);
  assert.notEqual(draft.keyPointsNote, draft.viewingPointsNote);
  assert.notEqual(draft.viewingPointsNote, draft.basicDescription);
  ...
  assert.ok(result.slides.every((slide) => slide.movie.backdrop.endsWith("backdrop.jpg")), "5장 모두 대표 장면 자산을 보존해야 한다");
  ...
  assert.ok(result.slides[3].note, "4장에도 하단 요약 인용문이 실린다");
  assert.ok(result.slides[4].note, "5장 하단 요약 인용문");
  assert.notEqual(result.slides[3].note, result.slides[4].note);
}
