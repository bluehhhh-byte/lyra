...
assert.match(smoke, /\{ url: "\/admin", expect: "곡 추가" \}/, "스모크가 관리자 본문 기대값을 등록해야 한다");
assert.match(smoke, /body\.includes\(page\.expect\)/, "스모크가 관리자 Server Component 본문을 확인해야 한다");
assert.match(smoke, /BASE \+ "\/api\/admin\/deploy"/, "스모크가 로그인 후 관리자 배포 API 서버 청크를 불러와야 한다");
assert.match(deploy, /wait-for-production\.mjs/, "운영 배포의 정확한 커밋 반영을 기다려야 한다");
assert.match(deploy, /scripts\/verify-production\.mjs/, "배포 후 운영 검증 스크립트를 실행해야 한다");
assert.match(deploy, /Global\\LyraProductionDeploy/, "동시 배포를 막는 프로세스 잠금이 필요하다");
assert.match(deploy, /\$liveSha -eq \$targetSha/, "이미 운영 중인 같은 커밋은 다시 올리면 안 된다");
