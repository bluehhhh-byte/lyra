...
export function genreStatus(raw) {
  ...
  const locked = /^true$/i.test(fieldOf(m[1], "genre_locked"));
  const issue = genreIssue(tagGenre, locked);
  return {
    field,
    tagGenre,
    issue,                                   // 태그 장르 자체의 문제 (없음·비표준·세분화 권장)
    locked,                                  // Jev가 세분화 불가를 확인해 잠긴 장르인가
    drift: !!(tagGenre && field && tagGenre !== field), // 두 값이 어긋남
  };
}

// 사람이 Rock/Pop을 다시 골라 저장했는데 Jev(lib/admin/jev.js의
// checkGenreSubdividable)가 "이 곡은 정말 더 세분화될 수 없다"고 독립적으로
// 확인했을 때만 호출된다(app/api/admin/songs.js의 genreApply 참조).
// genreIssue()가 이 플래그를 보고 "세분화 권장"을 다시 매기지 않아,
// 10cc <I'm Not In Love>처럼 정말 순수한 곡이 "장르 손보기" 대기열에서
// 영원히 못 빠지는 사고가 재발하지 않는다.
export function lockGenre(raw) {
  const text = String(raw || "").replace(/\r\n/g, "\n");
  const m = text.match(FM);
  if (!m) throw new Error("frontmatter를 읽을 수 없습니다.");
  const out = setField(text, "genre_locked", "true", "genre");
