...
const safeLimit = (value) => Math.floor(Math.max(0, Math.min(500, Number.isFinite(Number(value)) ? Number(value) : DEFAULT_APPEARANCE_AI_DAILY_LIMIT)));
...
export function researchBudgetStatus(value, { limit = DEFAULT_APPEARANCE_AI_DAILY_LIMIT, at = new Date() } = {}) {
  ...
  const dailyLimit = safeLimit(limit);
