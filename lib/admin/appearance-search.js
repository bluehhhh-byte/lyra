...
export function pickCandidate(parsed, candidates) {
  ...
  if (!want) return null;
  return (
    candidates.find((c) => normalizeTitle(c.work) === want) ||
    candidates.find((c) => normalizeTitle(c.work).includes(want) || want.includes(normalizeTitle(c.work))) ||
    null
  );
}

export async function suggestFromFreeSources({ key, title, artist, lang, geminiText }, options = {}) {
  const candidates = await findAppearanceCandidates({ artist, title, lang }, options);
  // "찾지 못함"을 "없음"으로 내보내지 않는다. 이 경로의 재현율은 22%라, 빈손이
   // 곧 수록 정보가 없다는 뜻이 될 수 없다. needs_review를 쓰면 화면이 경고
  // 문구를 그대로 보여 주고 사람이 직접 확인하게 된다.
  if (!candidates.length)
    return {
      appearance: null,
      state: "needs_review",
      warning: "Apple Music·위키백과에서 수록 정보를 찾지 못했습니다. 수록된 작품이 없다는 뜻은 아니므로 직접 확인해 주세요.",
      candidates: [],
    };

  const prompt = `아래는 노래 "${title}" — ${artist}가 어떤 작품에 쓰였는지에 대한 검색 후보다.
각 후보는 실제 출처에서 온 것이고, 근거 URL은 이미 확보돼 있다.

후보:
${candidates.map((c, i) => `${i + 1}. 작품 후보: ${c.work}\n   출처: ${c.label}\n   근거 문장: ${c.claim}`).join("\n")}

이 중 이 곡이 실제로 쓰인 작품 하나를 고르고, JSON만 출력해라. 확신이 없으면 work를 빈 문자열로 둔다.
{"work":"후보 중 하나의 작품명 그대로","workType":"movie|drama|anime_movie|anime_series|documentary|game|other","role":"main_theme|opening|ending|insert_song|background|trailer|other","year":숫자 또는 null,"note":"한국어 한 문장","evidenceClaim":"곡과 작품의 관계를 설명하는 한국어 한 문장"}
...
- 사운드트랙 앨범 수록만 확인된 경우 role은 background로 둔다 — 주제가라고 단정하지 마라.`;
  ...
  return {
    ...
    appearance: {
      ...
      evidenceClaim: String(parsed.evidenceClaim || chosen.claim).slice(0, 300),
      status: "verified",
    },
  };
}
