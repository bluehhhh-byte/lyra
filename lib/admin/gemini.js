...
export async function geminiGrounded(key, prompt, model = GEMINI_MODEL) {
  ...
  for (const m of [model, ...FALLBACK_MODELS]) {
    ...
    tried.add(m);
    if (Date.now() >= deadline) break;
    const result = await callModel(apiKey, prompt, false, clean(m), deadline, { grounded: true });
    if (result) return result;
  }
  return null;
}

async function callModel(key, prompt, json, model, deadline, { grounded = false, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
