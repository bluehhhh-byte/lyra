...
export async function geminiText(key, prompt, json = false, model = GEMINI_MODEL) {
  ...
  for (const m of [model, ...FALLBACK_MODELS]) {
    ...
    const text = await callModel(apiKey, prompt, json, clean(m), deadline);
    if (text) return text;
  }
  return "";
}
...
export async function geminiGrounded(key, prompt, model = GEMINI_MODEL) {
  ...
  for (const m of [model, ...FALLBACK_MODELS]) {
    ...
    tried.add(m);
    if (Date.now() >= deadline) break;
    const result = await callModel(apiKey, prompt, false, clean(m), deadline, true);
    if (result) return result;
  }
  return null;
}

async function callModel(key, prompt, json, model, deadline, grounded = false) {
