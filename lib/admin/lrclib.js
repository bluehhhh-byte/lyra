...
// nativeMeta의 반대편. 미국 스토어는 같은 트랙을 로마자로 내준다(脈 → MYAKU,
// アクロの丘 → Akuro No Oka). 관리자의 등록곡 검색은 제목·아티스트·앨범 글자만
// 보므로, 일본 곡은 일본어로 쳐야만 찾혔다. 이 표기를 검색 별칭으로 저장해 두면
// 로마자로도 찾힌다 — 음역을 우리가 만들 필요가 없다.
export async function romanizedTitle(trackId, left = () => 8000) {
  if (!trackId || left() <= 0) return "";
  const j = await getJson(
    `https://itunes.apple.com/lookup?${new URLSearchParams({ id: trackId, country: "US" })}`,
    Math.min(4000, left())
  );
  const title = (j?.results?.[0]?.trackName || "").trim();
  return title && !hasCJK(title) && /\p{Script=Latin}/u.test(title) ? title : "";
}
