...
export const APPLE_SEARCH_CACHE_SECONDS = 3600;
...
export async function searchItunesStorePage(query, country, { limit = 50, offset = 0, fetchImpl = fetch } = {}) {
  ...
  try {
    const response = await fetchImpl(`https://itunes.apple.com/search?${params}`, {
      ...
      // Apple은 호출량을 약 분당 20회로 제한한다. POST 관리자 라우트 안의
      // 하위 fetch도 명시적으로 Data Cache에 넣어 인스턴스가 바뀌어도 같은
      // 국가·질의·페이지를 한 시간 동안 Apple에 다시 보내지 않는다.
      cache: "force-cache",
      next: { revalidate: APPLE_SEARCH_CACHE_SECONDS },
    });
...
export async function fetchArtistCatalog(query, { fetchImpl = fetch } = {}) {
  ...
  await Promise.all(
    terms.flatMap((term) =>
      ["KR", "US"].map((c) =>
        fetchImpl(`https://itunes.apple.com/search?term=${encodeURIComponent(term)}&entity=musicArtist&limit=3&country=${c}`, {
  ...
  const lists = await Promise.all(
    ids.slice(0, 3).map((id) =>
      fetchImpl(`https://itunes.apple.com/lookup?id=${id}&entity=song&limit=200&country=KR`, {
        signal: AbortSignal.timeout(3200),
        cache: "force-cache",
        next: { revalidate: APPLE_SEARCH_CACHE_SECONDS },
      })
