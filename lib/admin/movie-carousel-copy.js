...
5) keyPointsNote: 1문장. keyPoints 세 항목을 하나로 누른 요약 — 이 영화의 서사가 무엇의 이야기인지 한 줄로 말한다. basicDescription을 반복하지 않는다. 30~110자.
6) viewingPointsNote: 1문장. viewingPoints 세 항목을 하나로 누른 요약 — 장르·국가·연도의 맥락에서 이 영화가 어떤 자리에 있는지 한 줄로 말한다. basicDescription을 반복하지 않는다. 30~110자.

근거 자료:
${sourceText(movie).slice(0, 5000)}

순수 JSON만 출력:
{"basicDescription":"...","synopsis":"...","keyPoints":["...","...","..."],"viewingPoints":["...","...","..."],"keyPointsNote":"...","viewingPointsNote":"..."}`;
}
...
export function parseMovieCarouselCopy(raw, movie = null) {
  ...
  const copy = {
    ...
    // 예전 스키마(closingNote 하나)로 답해도 받아 준다 — 5장 요약 자리로 들어간다.
    keyPointsNote: clean(value?.keyPointsNote),
    viewingPointsNote: clean(value?.viewingPointsNote || value?.closingNote),
  };
  if (!lengthIn(copy.basicDescription, 40, 280) || !lengthIn(copy.synopsis, 80, 600)) return null;
  if (!lengthIn(copy.keyPointsNote, 20, 160) || !lengthIn(copy.viewingPointsNote, 20, 320)) return null;
  ...
  if ([copy.basicDescription, copy.synopsis, copy.keyPointsNote, copy.viewingPointsNote, ...points].some((text) => !grounded(text, movie))) return null;
  return copy;
}
