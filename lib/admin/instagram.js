...
// 줄 끝 CR도 같이 턴다: 캡션은 CRLF로 저장돼 있고 곡 파일은 LF라, 남겨 두면
// 나중에 원본과 대조할 때 모든 줄이 다르게 보인다.
export const fixMojibake = (s) => Buffer.from(s || "", "latin1").toString("utf8").replace(/\r\n?/g, "\n");
...
export function parseHeader(caption) {
  let first = normalizeHeaderLine(caption.split("\n")[0]);
  // 헤더 뒤에 큰따옴표로 한 줄 요약을 붙인 글이 많다:
  //   `| MGMT - Piece of What (2006) “사랑조차 지나가버린 자리…”`
  // 그대로 두면 요약문이 통째로 제목이 된다. 떼어내 summary로 돌려준다.
  let summary = "";
  const q = first.match(/\s*[“"]([^”"]{10,})[”"]\s*$/);
  if (q) {
    summary = q[1].trim();
    first = first.slice(0, q.index).trim();
  }
  ...
  return { artist: m[1].trim(), title: m[2].trim(), title_ko, year, summary };
}
...
export function planImport(posts, existing = []) {
  ...
  for (const post of posts) {
    ...
    if (lines.filter((l) => l.trim()).length < 4) {
      // 요약(헤더 뒤 큰따옴표 한 줄)도 같이 남긴다 — 나중에 코멘트로 쓸 수 있다
