...
export const TITLE_KO_RULE =
  "titleKo: 곡 제목을 한국어로 옮긴다. 원칙은 뜻으로 번역하는 것이다" +
  "(Green→초록, Moonlight→달빛, Footsteps→발소리, It's Late→늦었어). " +
...
export const commentPrompt = (title, artist, lyrics) => {
  ...
  const basis = lyricText
    ? `가사의 의미와 이 곡에 얽힌 실제 배경·일화를 녹여서.\n가사:\n${lyricText.slice(0, 2000)}`
    : "가사가 없는 연주곡이거나 원문을 확인할 수 없는 곡이다. 가사를 지어내거나 가사의 의미를 언급하지 말고, 곡의 구성·연주·음색·분위기와 알려진 실제 배경을 바탕으로 써줘.";
  return `노래 "${title}" (${artist})에 대한 개인 음악 블로그용 코멘트를 한국어 1~2문장으로 써줘. ${basis} 반드시 평서문 '~다'체(예: ~한다, ~이다, ~같다, ~된다)로 끝맺을 것. "~습니다/~합니다/~해요/~함/~음" 금지. 담백한 톤. 코멘트 문장만 출력.`;
};
...
//
// 한 줄 안에서 언어가 섞이면 조각별로 교차한다. 예전에는 주 언어로 줄 전체를 옮겨서
// "All I can do is try, 우리만에 시간" 이 "All I can do is try—the time that was only
// ours" 가 됐다 — 한국어 독자가 못 읽던 영어 절반이 그대로 영어로 남았다.
...
- ${TITLE_KO_RULE}
- artistKo: 아티스트명이 일본어/한자면 한글 독음, 그 외에는 빈 문자열
- comment: 이 곡에 대한 담백한 감상 1~2문장. 반드시 평서문 '~다'체로 끝맺을 것("~습니다/~합니다/~해요/~함/~음" 금지). 확실히 아는 배경이나 일화가 있으면 녹이고, 모르면 지어내지 말고 제목·아티스트·장르·발매 시기${instrumental ? "·구성·연주·음색" : ""}에서 읽히는 인상만 적어라. 가사 내용을 아는 척하지 마라
