...
import { checkCliche, CLICHE_THRESHOLD, reviewSongMeta, LISTEN_CLICHE_THRESHOLD, EMOTION_MISMATCH_MAX, EMOTION_SUGGEST_MIN } from "./jev.js";
...
- A line mixing languages → CROSS-TRANSLATE it: every segment flips to the other side. The Korean parts become English, and the English (or Japanese) parts become Korean, in one natural line. Judging by a dominant language would leave the other half sitting in the language it was already in, which is the half the reader could not read to begin with.
  ...
  A foreign word embedded in a mostly-Korean line MUST flip too, even when leaving it alone would read perfectly well in the English output — that word is the whole reason the line needed cross-translation.
  "먼저 말하고 싶은 내 goodbye, 길을 잃어" → "> My 작별 that I want to say first, I've lost my way"  (NOT "My goodbye that I want to say first")
  Keep the original order of the parts when it still reads naturally; reorder only when the flipped languages demand it.

Rules:
- Direction is decided PER LINE. The primary language hint never flips a line's direction: a Korean line ALWAYS gets " > " + a natural English translation, even when the hint is en or ja; an English line ALWAYS gets " > " + a natural Korean translation.
- A line mixing Korean and English is CROSS-TRANSLATED as described above: each segment flips to the other language. Never translate a mixed line wholesale into a single language.
...
// Jev(lib/admin/jev.js)로 상투구 확률을 물어, 임계값을 넘으면 딱 한 번만
// 재작성을 시도한다. Jev가 응답을 못 주면(키 없음·프로모션 종료 후 402 등)
// checkCliche가 null을 돌려주므로 그대로 원래 코멘트를 쓴다 — 이 함수는
// 어떤 경로로도 예외를 던지지 않고, 실패할수록 원래 동작(게이트 없음)에
// 가까워진다.
async function gatedComment(comment, { title, artist, lyrics, key }) {
  if (!comment || !lyrics || !key) return comment;
  const p = await checkCliche(comment);
  if (p === null || p < CLICHE_THRESHOLD) return comment;
  const retryPrompt =
    commentPrompt(title, artist, lyrics) +
    ` 방금 쓴 코멘트("${comment}")는 "그립다/애절하다/쓸쓸하다/먹먹하다" 같은 상투적 감성 어휘에 기댄다는 평가를 받았다. 그 단어들 없이 곡의 구체적 이미지나 상황으로 다시 써라.`;
  const retried = await geminiText(key, retryPrompt);
  return retried ? retried.replace(/\s*\n+\s*/g, " ").trim() : comment;
}

// Jev(lib/admin/jev.js의 reviewSongMeta)가 listen_when 상투구와 감정 불일치를
// 호출 한 번에 본다. listen_when이 흔한 장면이면 한 번만 다시 쓴다. 감정은 Jev가
// 직접 바꾸지 않는다 — 기존 감정의 확률이 거의 0이고 다른 감정이 뚜렷할 때만,
// 가사를 보는 Gemini에게 둘 중 하나를 다시 고르게 한다. Jev가 응답을 못 주면
// (키 없음·프로모션 종료 등) 원래 값을 그대로 돌려준다.
async function gatedListenAndEmotion({ comment, listenWhen, emotion }, { title, artist, lyrics, key }) {
  if (!key || !lyrics) return { listenWhen, emotion };
  const review = await reviewSongMeta({ comment, listenWhen, emotions: EMOTIONS });
  if (!review) return { listenWhen, emotion };

  let nextListen = listenWhen;
  if (listenWhen && review.listenCliche !== null && review.listenCliche >= LISTEN_CLICHE_THRESHOLD) {
    const raw = await geminiText(
      key,
      `노래 "${title}" (${artist})의 커버 카드에 들어갈 "이런 순간에" 한 줄을 다시 써라. ` +
        `방금 쓴 문구("${listenWhen}")는 어느 곡에나 붙는 흔한 장면(혼자 있는 밤·잠 못 드는 새벽 같은)에 ` +
        `기댄다는 평가를 받았다. 가사의 구체적 상황에서 출발해라.\n${LISTEN_WHEN_RULE()}\n` +
        `문구 한 줄만 출력.\n가사:\n${String(lyrics).slice(0, 1500)}`,
      false,
      GEMINI_LITE_MODEL
    );
    const cleaned = cleanListenWhen(raw);
    if (cleaned) nextListen = cleaned;
  }

  let nextEmotion = emotion;
  const probs = review.emotionProbs;
  if (emotion && probs && typeof probs[emotion] === "number" && probs[emotion] < EMOTION_MISMATCH_MAX) {
    const [suggested, p] = Object.entries(probs).sort((a, b) => b[1] - a[1])[0] || [];
    if (suggested && suggested !== emotion && p >= EMOTION_SUGGEST_MIN && parseEmotion(suggested)) {
      const raw = await geminiText(
        key,
        `노래 "${title}" (${artist})의 가사를 보고 곡의 감정으로 "${emotion}"과 "${suggested}" 중 ` +
          `더 알맞은 것 하나만 골라 그 단어만 출력해라.\n가사:\n${String(lyrics).slice(0, 1500)}`,
        false,
        GEMINI_LITE_MODEL
      );
      const picked = parseEmotion(String(raw || "").replace(/["'「」\s.]/g, ""));
      if (picked === suggested || picked === emotion) nextEmotion = picked;
    }
  }
  return { listenWhen: nextListen, emotion: nextEmotion };
}
...
${lyrics.slice(0, 2000)}`,
      ...
      comment = await gatedComment(comment, { title, artist, lyrics, key });
      ({ listenWhen, emotion } = await gatedListenAndEmotion({ comment, listenWhen, emotion }, { title, artist, lyrics, key }));
