...
// 줄마다 빈 줄이 끼어 한 줄이 곧 한 연이 되어 버린 가사를 붙인다.
// 다른 곡들은 연 안에서 줄이 연달아 붙고 빈 줄은 연 경계에만 있다 —
// 그 형태로 되돌린다. 빈 줄로만 연을 나눈 곡을 뭉개지 않도록, 부르는 쪽에서
// 구간 표시를 함께 넣을 때만 쓴다(구간이 연 경계를 대신하기 때문이다).
export function tightenStanzas(body) {
  const lines = String(body || "").split("\n");
  const kept = lines.filter((line) => line.trim() !== "");
  const out = [];
  for (const line of kept) {
    if (isSectionLine(line) && out.length) out.push("");
    out.push(line);
  }
  return out.join("\n");
}
...
export function applySections(body, marks = [], { tighten = false } = {}) {
  const lines = String(body || "").split("\n");
  ...
  return tighten ? tightenStanzas(joined) : joined;
}
