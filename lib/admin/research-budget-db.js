...
import { AI_PHASES, aiBudgetLimits, aiBudgetResetAt, aiBudgetStatus, isAiPhase } from "./ai-budget.js";
...
export async function readResearchDashboard(sql, {
  ...
  return {
    enabled: true,
    budget,
    // 예산이 언제 풀리는지는 화면이 스스로 계산하면 안 된다 — 브라우저 시간대가
    // KST가 아니면 거짓말이 된다. 서버가 KST로 셈해서 내려보낸다.
    resetAt: aiBudgetResetAt(at),
    checkpoint: row ? {
