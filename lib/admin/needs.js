...
export function needsLyricSections(song) {
  ...
  if (lyricLines.length < 20 || song.stanzas.some((stanza) => String(stanza.section || "").trim())) return false;
  const normalized = lyricLines.map((line) => line.toLocaleLowerCase().replace(/[\p{P}\p{S}\s]+/gu, ""));
  return new Set(normalized).size < normalized.length || lyricLines.length >= 32;
}
...
export function translationStatus(s) {
  ...
  const stanzas = s.stanzas.map((stanza) => ({
    ...
    lines: stanza.lines.map((line) => {
      const translationMissing = (checkKo && needsKo(line)) || (checkEn && needsEn(line, s.lang));
...
export const summarizeNeeds = (s) => {
  ...
  if (n.sections) out.push("가사 구간 없음");
  return out;
};
