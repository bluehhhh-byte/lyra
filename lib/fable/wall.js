...
export function createWall(container, options = {}) {
  ...
  function emit(y0, y1, draw) {
    ...
    for (let index = first; index <= last; index++) {
      ...
      tile.queue.push(draw);
      if (tile.active) tile.done = false;
    }
    ensureFrame();
  }

  // A bitmap costs widthUnits x tileHeight x pixelScale^2 x 4 bytes — about 9 MB
  // per tile on a phone, and a tall page has dozens. Tiles that leave the retain
  // window give their bitmap back; the draw queue is kept so they repaint on return.
  ...
  function activateVisibleTiles() {
    ...
    // Release a full viewport later than we allocate, so a tile sitting on the
    // boundary is not freed and re-drawn on every scroll event.
    const keep = lookahead + 1;
    for (const tile of tiles) {
      const bottom = tile.y0 + tile.height;
      const near = tile.y0 < top + view * lookahead && bottom > top - view * lookahead;
      if (!tile.active && near) {
        tile.active = true;
        allocateTile(tile);
        ensureFrame();
      } else if (tile.active && (tile.y0 > top + view * keep || bottom < top - view * keep)) {
        releaseTile(tile);
      }
    }
  }
  ...
  // Idle means every visible tile has drained its queue and generation is over.
  // The loop then stops; scrolling and emit() wake it again through ensureFrame.
  function ensureFrame() {
    if (!dead && !frame) frame = requestAnimationFrame(tick);
  }
  ...
  function tick() {
    ...
    for (const tile of tiles) {
      ...
      // Queue is retained, not nulled — releaseTile replays it when the tile returns.
...
// Safari hands back a blank canvas once the backing store passes a few
// megapixels, and every browser caps a single side. A long lyrics column is tall
// enough to hit both, so the resolution is scaled down rather than the drawing
// dropped — the marks are strokes, not photos, and degrade gracefully.
const MAX_CANVAS_AREA = 4e6;
