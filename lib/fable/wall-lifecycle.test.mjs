...
test("캔버스 백킹스토어는 면적·한 변 상한을 넘지 않는다", async () => {
  installDom();
  const { createCanvasSurface } = await import("./wall.js");

  const surfaceFor = (width, height) => {
    const canvas = makeCanvas();
    canvas.getBoundingClientRect = () => ({ width, height });
    createCanvasSurface(canvas, { widthUnits: width, heightUnits: height });
    return canvas;
  };

  // 짧은 곡 — 상한에 걸리지 않으니 devicePixelRatio 그대로.
  const small = surfaceFor(700, 900);
  assert.equal(small.width, 700);
  assert.equal(small.height, 900);

  // 긴 가사 컬럼 — 예전에는 8.4Mpx 비트맵을 요구해 iOS 에서 빈 화면이 됐다.
  const tall = surfaceFor(700, 12000);
  assert.ok(tall.width >= 1 && tall.height >= 1, "축소해도 0 이 되면 안 된다");
  assert.ok(tall.width * tall.height <= 4e6 + 1, `면적 상한 초과: ${tall.width * tall.height}`);
  assert.ok(Math.max(tall.width, tall.height) <= 8192, "한 변 상한 초과");
  // 종횡비는 유지돼야 그림이 찌그러지지 않는다.
  assert.ok(Math.abs(tall.width / tall.height - 700 / 12000) < 0.001);
});
