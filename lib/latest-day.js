...
// 영화도 곡과 같은 15어휘의 감정을 가진다(브리프 §10 Phase 3) — 감정이 붙은
// 기록은 종류를 가리지 않고 좌표 계산에 들어가고, 없는 기록은 세지 않는다.
...
const normalizeEntries = (songs, movies) => [
    ...
    ...movies.map((m) => ({
      ...
      emotion: parseEmotion(m.emotion),
      themes: m.themes || [],
...
function insightForDay(day, items) {
  ...
  const emotions = tally(items.map((e) => e.emotion));
  const center = emotionCenter(items.map((e) => e.emotion));
...
// 홈의 재발견 모듈 — 해가 다른 같은 월-일의 기록 (브리프 §10 Phase 2).
// 965곡 아카이브의 홈이 "최근"만 보여주는 최근성 편향을 깬다. 연도당 한 건씩
// 가까운 해부터 담고, 올해 기록은 뺀다(상단 분석이 이미 다룬다). 과거 기록이
// 없는 날은 빈 배열 — 모듈째 사라진다. 억지로 채우지 않는다.
export function anniversaryRecords(songs = [], movies = [], todayDay = "", limit = 3) {
  const monthDay = String(todayDay).slice(5);
  const thisYear = Number(String(todayDay).slice(0, 4));
  if (monthDay.length !== 5 || !thisYear) return [];
  const matches = [
    ...songs.map((s) => ({ type: "song", slug: s.slug, title: s.title, subtitle: s.artist, at: recordedAt(s), day: kstDay(recordedAt(s)) })),
    ...movies.map((m) => ({ type: "movie", slug: m.slug, title: m.title_ko || m.title, subtitle: m.director_ko || m.director || "", at: recordedAt(m), day: kstDay(recordedAt(m)) })),
  ].filter((r) => r.day && r.day.slice(5) === monthDay && Number(r.day.slice(0, 4)) < thisYear);

  const byYear = new Map();
  // 같은 해에 여럿이면 그날 늦게 남긴 기록 — 하루의 마지막 인상이 그날을 대표한다
  for (const record of matches.sort((a, b) => String(b.at).localeCompare(String(a.at)))) {
    const year = Number(record.day.slice(0, 4));
    if (!byYear.has(year)) byYear.set(year, { ...record, year });
  }
  return [...byYear.values()].slice(0, limit);
}
...
export function dayInsights(songs = [], movies = []) {
  ...
  const reports = [...byDay.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([day, items]) => insightForDay(day, items));

  // 작품은 개별 날짜의 해시만으로 고르면 서로 다른 입력도 같은 나머지값에 걸려
  // 며칠 연속 같은 지시문이 나올 수 있다. 앞선 '기록일' 세 날의 작품을 후보에서
  // 제외해, 날짜가 띄엄띄엄 있어도 연속으로 보는 네 작품의 패턴이 모두 다르게 한다.
  const recentScores = [];
  const recentPatterns = [];
  for (const report of reports) {
    report.artworkAvoid = [...recentScores];
    report.artworkPatternAvoid = [...recentPatterns];
    const plan = latestArtworkPlan(report);
    recentScores.unshift(plan.score);
    if (recentScores.length > 3) recentScores.pop();
    recentPatterns.unshift(plan.patternId);
    if (recentPatterns.length > 40) recentPatterns.pop();
  }
  return reports;
}
