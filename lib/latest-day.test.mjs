...
const song = (slug, published, emotion, extra = {}) => ({
  slug, title: `T-${slug}`, artist: `A-${slug}`, published, emotion, genre: "Indie Pop", keywords: ["밤", "꿈"], ...extra,
});
...
// 최신 날짜를 songs와 movies 전체에서 고르고, 그 날 항목만 함께 집계한다
{
  ...
  assert.match(r.text, /Indie Pop의 질감/);
  assert.match(r.text, /‘꿈’|‘밤’/);
  assert.doesNotMatch(r.text, /표본|밝기|각성|좌표|음악 3곡/);
  assert.ok(r.text.length <= 220, `평론은 담백하게 유지한다: ${r.text.length}자`);
...
// 최신 하루만 버리지 않고 날짜별 리포트를 오래된 순서로 모두 돌려준다
{
  ...
  assert.match(reports[0].text, /Indie Pop/);
  assert.match(reports[1].text, /M-middle/);
  assert.match(reports[2].text, /Indie Pop/);
}
console.log("✓ 날짜별 리포트 누적");

// 곡이 하나뿐이어도 표본·좌표 설명 없이 음악 평론으로 쓴다
{
  const r = latestDayInsight([song("a", "2026-08-15T00:00:00Z", "불안")], []);
  assert.equal(r.sparse, true);
  assert.match(r.text, /Indie Pop/);
  assert.doesNotMatch(r.text, /표본|한 곡|하루의 흐름|밝기|각성/);
}
console.log("✓ 한 곡도 짧은 음악 평론");
...
// 서술 재료는 실제 가사와 장르이며, 수치 분석 대신 두 문장 평론으로 쓴다
{
  const r = latestDayInsight([
  ...
  assert.match(r.text, /Dream Pop·Electronic의 질감/);
  assert.match(r.text, /검은 파도가 계속 밀려온다/);
  assert.match(r.text, /새벽이 올 때까지 문 앞에서 기다린다/);
  assert.doesNotMatch(r.text, /설명글에서는|표본|밝기|각성|좌표|\d곡/);
  assert.ok(r.text.length <= 220, `두 문장 평론은 220자를 넘지 않는다: ${r.text.length}자`);
  assert.equal((r.text.match(/\./g) || []).length, 2, "음악 평론은 두 문장으로 끝낸다");
}
console.log("✓ 오늘의 기록 — 핵심 가사·장르 중심의 짧은 평론");
