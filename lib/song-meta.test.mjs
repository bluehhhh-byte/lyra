...
assert.match(prompt, /웹 근거 없는 외부 배경/);
assert.doesNotMatch(prompt, /가사의 의미와/);

const lyricPrompt = commentPrompt("Lose Control", "L'Arc-en-Ciel", "I lose control");
assert.match(lyricPrompt, /웹 근거가 제공되지 않았으므로/);
assert.match(lyricPrompt, /영화·드라마 사용/);
