...
test("the signature is dimmer than the wordmark it follows", () => {
  ...
  const block = chrome.slice(chrome.indexOf("export function drawSignature"), chrome.indexOf("export function drawLastSlideFooter"));
...
// ── 영화 카드도 같은 푸터 ─────────────────────────────────────────────────
test("the movie deck asks on its last slide too, in its own words", () => {
  assert.match(MOVIE_CTA_PRIMARY, /프로필 링크/);
  assert.match(MOVIE_CTA_SECONDARY, /저장/);
  // 가사 문구를 그대로 쓰면 영화 카드가 "이 가사가 좋았다면"이라고 말한다
  assert.notEqual(MOVIE_CTA_PRIMARY, CAROUSEL_CTA_PRIMARY);
  assert.ok(!/가사/.test(MOVIE_CTA_PRIMARY + MOVIE_CTA_SECONDARY));
  // 렌더러가 둘(단일·큐레이션)이라 각자 끝맺으면 한쪽만 붙는다
  assert.match(studio, /function finish\(ctx, position\) \{/);
  assert.match(studio, /if \(position === TOTAL_SLIDES\)\s*\n\s*drawLastSlideFooter/);
  assert.equal((studio.match(/\n  finish\(ctx, position\);/g) || []).length, 2, "5장이 될 수 있는 렌더러 둘 다");
});

test("the movie slides that can be last leave the footer its room", () => {
  // 영화 카드는 가사 카드와 달리 본문 y가 고정이다. 푸터를 얹으면서 위쪽을
  // 비우지 않으면 권유 문구가 인용·본문 상자 위에 그대로 겹친다.
  const { dividerY } = carouselFooterLayout();
  // 감상 포인트(5장): 마지막 상자가 1035에서 끝나고 인용 두 줄이 그 아래 들어간다
  const quote = studio.match(/ctx\.fillText\(line, PAD, (\d+) \+ index \* 38\)/);
  assert.ok(quote, "인용 줄의 y를 찾지 못했다");
  const lastQuoteBaseline = Number(quote[1]) + 38;
  assert.ok(lastQuoteBaseline < dividerY, `인용 ${lastQuoteBaseline} ≥ 구분선 ${dividerY}`);
  // 선정 노트(5장): 상자 높이를 줄여 구분선 위에서 끝난다
  const panel = studio.match(/ctx\.rect\(PAD, 760, W - PAD \* 2, (\d+), 28\)/);
  assert.ok(panel, "선정 노트 상자를 찾지 못했다");
  assert.ok(760 + Number(panel[1]) <= dividerY, `상자 바닥 ${760 + Number(panel[1])} > 구분선 ${dividerY}`);
});
