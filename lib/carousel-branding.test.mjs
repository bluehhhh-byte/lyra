...
test("the signature is dimmer than the wordmark it follows", () => {
  ...
  const block = chrome.slice(chrome.indexOf("export function drawSignature"), chrome.length);
...
test("the last slide says nothing extra at all", () => {
  // 마지막 장에 차례로 붙었다가 차례로 뺀 것들: 권유 두 줄과 구분선, 그리고
  // "… 이하 생략". 카드는 가사를 보여 주는 자리지 말을 거는 자리가 아니다.
  for (const [name, source] of [["가사 카드", card], ["영화 카드", studio], ["공용", chrome]]) {
    ...
    assert.ok(!/이하 생략/.test(source), `${name}에 생략 표시가 남아 있다`);
  }
  // 안 그리는 것을 위해 자리를 비워 두면 마지막 장 가사만 이유 없이 작아진다
  assert.match(card, /carouselBodyBox\(\)/, "예약 인자 없이 부른다");
  const box = carouselBodyBox();
  assert.equal(box.height, 1350 - 360, "마지막 장도 같은 높이를 쓴다");
  // 영화 카드도 푸터에 내줬던 자리를 돌려받았다
...
test("every prop the card renderers read is actually declared in scope", () => {
  ...
  // 그 함수의 매개변수에 있는지 본다.
  const named = ["hashtagSets", "isLast"];
