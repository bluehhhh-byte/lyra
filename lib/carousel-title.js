...
const QUALIFIER_SIZES = [25, 23, 21, 19];
...
export function layoutCarouselQualifier(value, measureAtSize, maxWidth, maxLines = 2) {
  const text = clean(value);
  if (!text) return { lines: [], fontSize: QUALIFIER_SIZES[0], lineHeight: 31, truncated: false };
  for (const fontSize of QUALIFIER_SIZES) {
    const measure = (line) => measureAtSize(line, fontSize);
    const lines = wrapTitle(text, measure, maxWidth);
    if (lines.length <= maxLines) {
      return { lines, fontSize, lineHeight: fontSize + 7, truncated: false };
    }
  }
  const fontSize = QUALIFIER_SIZES.at(-1);
  const measure = (line) => measureAtSize(line, fontSize);
  const wrapped = wrapTitle(text, measure, maxWidth);
  const lines = wrapped.slice(0, maxLines);
  lines[maxLines - 1] = ellipsize(wrapped.slice(maxLines - 1).join(" "), measure, maxWidth);
  return { lines, fontSize, lineHeight: fontSize + 7, truncated: true };
}
