...
async function neonSnapshot() {
  ...
  const [building] = await sql`select * from lyra_deploy_jobs where status = 'BUILDING' limit 1`;
  const [latest] = await sql`select * from lyra_deploy_jobs order by updated_at desc limit 1`;
  const [lastReady] = await sql`select * from lyra_deploy_jobs where status = 'READY' order by updated_at desc limit 1`;
  return { current: rowView(building || latest), lastReady: rowView(lastReady) };
}
...
const memView = (r) => r && ({ commitSha: r.commitSha, status: r.status, deploymentId: r.deploymentId, error: r.error, updatedAt: r.updatedAt });
...
function memSnapshot(now = Date.now()) {
  memSweep(now);
  let building = null;
  let latest = null;
  let lastReady = null;
  for (const row of mem.values()) {
    if (row.status === BUILDING) building = row;
    if (!latest || row.updatedAt > latest.updatedAt) latest = row;
    if (row.status === READY && (!lastReady || row.updatedAt > lastReady.updatedAt)) lastReady = row;
  }
  return { current: memView(building || latest), lastReady: lastReady && memView(lastReady) };
}

export function deploymentStatusSummary({ sourceSha = "", liveVersion = null, lastReady = null } = {}) {
  const validSha = (value) => (/^[0-9a-f]{40}$/i.test(String(value || "")) ? String(value).toLowerCase() : "");
  const source = validSha(sourceSha);
  const liveFromVersion = validSha(liveVersion?.sha);
  const liveFromLedger = liveVersion?.deploymentId && liveVersion.deploymentId === lastReady?.deploymentId
    ? validSha(lastReady?.commitSha)
    : "";
  const deployed = liveFromVersion || liveFromLedger;
  return {
    sourceSha: source,
    deployedSha: deployed,
    matches: source && deployed ? source === deployed : null,
    lastDeployedAt: lastReady?.updatedAt || null,
    deploymentId: liveVersion?.deploymentId || lastReady?.deploymentId || "",
  };
}
...
// 배포 화면용 — 진행 중/최근 작업과 마지막 성공 배포를 함께 읽는다.
export const deployStatusSnapshot = () => (useDb() ? neonSnapshot() : Promise.resolve(memSnapshot()));
