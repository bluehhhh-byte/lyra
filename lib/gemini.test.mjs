...
import { geminiGrounded, geminiText, withReason, GEMINI_MODEL, GROUNDING_UNAVAILABLE, groundingBlocked, resetGroundingBlock } from "./admin/gemini.js";
...
// 그라운딩 429는 계정·요금제 수준이라 모델을 바꿔도 같다. 사슬을 돌면 시간만
// 버린다 — 첫 429에서 멈춰야 한다.
{
  resetGroundingBlock();
  const tried = [];
  ...
  assert.match(withReason("리서치 실패"), /무료 티어/, "모델 탓으로 읽히지 않는 문구여야 한다");
  assert.doesNotMatch(withReason("리서치 실패"), /gemini-[\w.-]+: 429/, "모델 이름만 적으면 다른 모델을 시도하게 만든다");
  // 뒤따르는 검사가 래치에 걸리지 않게 되돌린다
  resetGroundingBlock();
}
...
// 그라운딩 차단은 요금제에서 온다 — 무료 티어 Gemini 3.x에는 Search 그라운딩이
// 없고(공식 가격표), 그것이 무료인 2.5는 신규 키에 404다. 그래서 다음 호출도
// 반드시 429고, 그 왕복 동안 관리자 화면은 "검색 중"으로 돈다. 한 번 확인했으면
// 같은 프로세스에서 다시 두드리지 않는다.
{
  resetGroundingBlock();
  assert.equal(groundingBlocked(), false, "시작은 열려 있다");

  let calls = 0;
  await withFetch(
    async () => { calls++; return Response.json({ error: { code: 429 } }, { status: 429 }); },
    async () => {
      assert.equal(await geminiGrounded("k", "질문"), null);
      assert.equal(calls, 1);
      // 두 번째 호출은 네트워크를 쓰지 않는다
      assert.equal(await geminiGrounded("k", "질문"), null);
      assert.equal(calls, 1, "막힌 것을 알고도 다시 두드렸다");
    }
  );
  assert.ok(groundingBlocked());

  // 문구가 재시도를 권하면 안 된다 — 예전 "한도가 초기화된 뒤 다시 시도하세요"가
  // 사용자를 무한 재시도로 몰았다.
  assert.match(GROUNDING_UNAVAILABLE, /무료 티어/);
  assert.match(GROUNDING_UNAVAILABLE, /직접 입력/);
  assert.ok(!/다시 시도하세요/.test(GROUNDING_UNAVAILABLE), "재시도를 권하지 않는다");

  // 요금제가 바뀌거나 키를 갈면 풀려야 한다 — 영구 잠금이 아니다
  resetGroundingBlock();
  assert.equal(groundingBlocked(), false);
}
console.log("✓ gemini — 그라운딩은 요금제에서 막힌다, 재시도가 아니라 안내가 답");
