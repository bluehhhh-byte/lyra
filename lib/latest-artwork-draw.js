...
export function latestArtworkTasks({ hand, plan, W, H, random }) {
  ...
  const M = 14;
  const composition = plan.composition || { index: 0, layoutIndex: 0, x: 0.5, y: 0.5, width: 0.94, height: 0.68, density: 1, angle: 0 };
  const BX0 = M + 18, BX1 = W - M - 18, BY0 = M + 18, BY1 = H - M - 18;
  const BW = BX1 - BX0, BH = BY1 - BY0;
  const frameW = BW * composition.width, frameH = BH * composition.height;
  const frameCx = BX0 + BW * composition.x, frameCy = BY0 + BH * composition.y;
  const X0 = frameCx - frameW / 2, X1 = frameCx + frameW / 2, Y0 = frameCy - frameH / 2, Y1 = frameCy + frameH / 2;
  const IW = X1 - X0, IH = Y1 - Y0;
  const inner = [[X0, Y0], [X1, Y0], [X1, Y1], [X0, Y1]];
  const turn = ([x, y]) => {
    const dx = x - frameCx, dy = y - frameCy;
    const cos = Math.cos(composition.angle), sin = Math.sin(composition.angle);
    return [
      clamp(frameCx + dx * cos - dy * sin, M + 2, W - M - 2),
      clamp(frameCy + dx * sin + dy * cos, M + 2, H - M - 2),
    ];
  };
  const at = (node) => {
    const [x, y] = turn([X0 + (node.x / 100) * IW, Y0 + (node.y / 100) * IH]);
    return { x, y };
  };
  ...
  if (score === "constellation") {
    ...
    tasks.push(() => hand.hatchFill(random, inner, 16 / composition.density, plan.angle + composition.angle, 0.05, { col: ink }));
    ...
    const step = 28 / composition.density;
    for (let row = 0; row < rows; row++) {
      const y = top + row * gapY;
      tasks.push(() => hand.scribFill(random, [[X0, y - 12], [X1, y - 12], [X1, y + 12], [X0, y + 12]].map(turn), 6, 0.05, { col: color, w: 2 }));
      tasks.push(() => hand.sk(random, [[X0, y + 12], [X1 - 14, y + 12]].map(turn), { col: ink, w: 0.9, a: 0.28, amp: 0.8, gap: 0.08 }));
      for (let x = X0 + 10; x < X1 - 16; x += step) {
        const [px, py] = turn([x, y]);
        if (plan.params.markKind === "spark") tasks.push(() => hand.spark(random, px + rd(random, -2, 2), py + rd(random, -2, 2), 10, { col: color, a: 0.88, w: 1.5, nR: 5, noCenter: true }));
        else if (plan.params.markKind === "ring") tasks.push(() => hand.ellipse(random, px, py, 9, 6, { col: color, w: 1.7, a: 0.85, amp: 0.7 }));
        else tasks.push(() => hand.sk(random, [[x, y - 9], [x + rd(random, -2, 2), y + 9]].map(turn), { col: color, w: 2.6, a: 0.8, taper: "out", amp: 0.5 }));
      }
      tasks.push(() => hand.sk(random, [[X1 - 6, y - 8], [X1 - 4, y + 10]].map(turn), { col: ink, w: 2.2, a: 0.7, taper: "out", amp: 0.3 }));
    }
  } else if (score === "solo") {
    const radius = Math.min(IW, IH) * 0.36;
    const cx = X0 + IW / 2, cy = Y0 + IH / 2;
    tasks.push(() => hand.hatchFill(random, inner, 15 / composition.density, plan.angle + composition.angle, 0.045, { col: ink }));
    ...
    const seamPoints = [];
    for (let y = Y0; y <= Y1; y += 12) seamPoints.push(turn([seamX + Math.sin(y * 0.05 + composition.index) * 6 + rd(random, -3, 3), y]));
    const leftPolygon = [[X0, Y0], ...seamPoints, [X0, Y1]].map((point, index) => index === 0 || index === seamPoints.length + 1 ? turn(point) : point);
    const rightPolygon = [[X1, Y0], ...seamPoints, [X1, Y1]].map((point, index) => index === 0 || index === seamPoints.length + 1 ? turn(point) : point);
    ...
    for (let index = 0; index < bands; index++) {
      ...
      tasks.push(() => hand.hatchFill(random, polygon.map(turn), 6.5 / composition.density, node.angle + composition.angle, 0.5, { col: paint(node.color) }));
      tasks.push(() => hand.sk(random, polygon.slice(0, 3).map(turn), { col: ink, w: 1.4, a: 0.4, amp: 1.2 }));
    }
    tasks.push(() => hand.sk(random, [[X0, Y1], [X1, Y1]].map(turn), { col: ink, w: 2.2, a: 0.7, amp: 0.6 }));
  } else if (score === "grid") {
    ...
    for (let index = 0; index < cells; index++) {
      ...
      tasks.push(() => hand.hatchFill(random, cell.map(turn), 6 / composition.density, directions[(col + row) % 4] + composition.angle, 0.55, { col: paint(node.color) }));
      tasks.push(() => hand.sk(random, [...cell, cell[0]].map(turn), { col: ink, w: 1.1, a: 0.5, amp: 0.6 }));
    }
  } else if (score === "rain") {
    tasks.push(() => hand.hatchFill(random, inner, 14 / composition.density, Math.PI / 2 + composition.angle, 0.04, { col: ink }));
    ...
    const [cx, cy] = turn([X0 + IW * clamp(plan.center.x / 100, 0.3, 0.7), Y0 + IH * clamp(plan.center.y / 100, 0.3, 0.7)]);
    ...
    tasks.push(() => hand.sk(random, [[X0, Y1], [X1, Y1]].map(turn), { col: ink, w: 1.6, a: 0.6, amp: 0.5 }));
    ...
    tasks.push(() => hand.sk(random, path.map(turn), { col: PALETTE.GOLD, w: 2.1, a: 0.85, amp: 1.3 }));
    tasks.push(() => hand.sk(random, path.map(([x, y]) => [x + 1.5, y + 2]).map(turn), { col: PALETTE.GOLD, w: 0.8, a: 0.3, amp: 1.8, gap: 0.15 }));
    points.forEach((point) => {
      const [x, y] = turn([point.x, point.y]);
      tasks.push(() => hand.enso(random, x, y, 7, 0.92, paint(point.node.color), { w: 3, a: 0.9 }));
    });
  }

  // 배치의 위치와 운동축을 화면에 남기는 공통 조형 부호. 선의 방향 네 종과
  // 매듭 수 다섯 종이 결합되어, 같은 지시문 안에서도 20개 구도를 눈으로 구별한다.
  const axisStart = turn([frameCx - IW * 0.43, frameCy]);
  const axisEnd = turn([frameCx + IW * 0.43, frameCy]);
  tasks.push(() => hand.sk(random, [axisStart, axisEnd], { col: ink, w: 0.8, a: 0.18, amp: 0.45, gap: 0.18 }));
  for (let index = 0; index <= composition.layoutIndex; index++) {
    const amount = (index + 1) / (composition.layoutIndex + 2);
    const x = axisStart[0] + (axisEnd[0] - axisStart[0]) * amount;
    const y = axisStart[1] + (axisEnd[1] - axisStart[1]) * amount;
    tasks.push(() => hand.enso(random, x, y, 3.2, 0.78, PALETTE.VIOLET, { w: 1.1, a: 0.42 }));
  }
