...
const [scenes, home, song, lyrics, wall, primitives] = await Promise.all([
  ...
  readFile(new URL("./fable/primitives.js", import.meta.url), "utf8"),
]);
...
assert.match(lyrics, /pb-3 pr-7 sm:pr-10/);
assert.match(scenes, /widthUnits - gutter/);
assert.match(scenes, /stanzaRects\.at\(-1\)\.bottom - parentRect\.top/);
assert.match(primitives, /points\.at\(-1\)\?\.\[1\] < end/);
