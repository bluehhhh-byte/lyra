...
// 카드는 발행물이고 곡 페이지는 기록이다 — 저장된 값을 바꾸는 것이 아니다
assert.match(card, /retimeListenWhen\(cleanListenWhen\(song\.listen_when\)\)/, "커버가 발행 시각에 맞춘다");
assert.ok(!/retimeListenWhen/.test(page), "곡 페이지는 기록 그대로 보여 준다");

console.log("✓ listen_when 정리 규칙 · 발행 시각 맞춤");
