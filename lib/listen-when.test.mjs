...
// Gemini 스키마와 배치 생성이 같은 규칙 문장을 쓴다 — 이제 호출마다 각도가 바뀌므로 함수로 부른다.
assert.match(LISTEN_WHEN_RULE(), /^listenWhen: /);
assert.match(LISTEN_WHEN_RULE(), /12~22자/);

// 시간대 각도가 나오더라도 매번 같은 낱말(밤)로 쏠리면 원래 문제가 재발한다 —
// 여러 번 불러 서로 다른 각도 지시문이 실제로 섞이는지 확인한다.
{
  const samples = new Set();
  for (let i = 0; i < 40; i++) samples.add(LISTEN_WHEN_RULE());
  assert.ok(samples.size >= 2, "40회 호출에서 각도가 최소 두 가지는 나와야 한다");
  for (const rule of samples) {
    assert.match(rule, /^listenWhen: /);
    assert.match(rule, /12~22자/);
  }
}
