...
test("a line that already has a translation is never stacked on", () => {
  // 한국어가 들어 있는 번역 칸은 needs.js가 "영어 번역 없음"으로 세지만, 그 위에
  // 영어를 쌓으면 파서가 두 `>` 줄을 하나로 합쳐
  // "Our own peculiar grammar 함께 괴상한 문법으로" 같은 반한글 번역이 된다.
  // IU <Dear my crazy soulmate>에서 실제로 그렇게 만들었다가 되돌렸다.
  // 방향이 틀린 줄은 지우거나 바꿔 쓰는 일이라 이 경로의 몫이 아니다.
  assert.match(block, /if \(translationUnder\(row\.index\)\) return false/);
});
...
test("the original lyrics and the frontmatter are never touched", () => {
  ...
  // 넣는 것은 `> ` 줄뿐이다
  assert.match(block, /lines\.splice\(at, 0, `> \$\{next\}`\)/);
  // 대입하는 자리는 번역 줄(translationLineIndex가 찾은 `>` 줄) 하나뿐이다.
  // 원문 줄에 대입하는 곳이 생기면 가사가 바뀐다.
  const assignments = block.match(/^\s*lines\[[^\]]+\] =/gm) || [];
  assert.deepEqual(
...
test("a failed Gemini call is not retried", () => {
  ...
  assert.match(block, /translated\.length !== jobs\.length/);
});

test("the per-song finisher replaces a Korean line sitting in the English slot", () => {
  // Effie <MAKGEOLLI BANGER>는 41줄 중 39줄이 영어인데 두 줄만 번역 칸에
  // 한국어가 들어 있었다. 일괄 도구는 덮어쓰기를 하지 않아 그 두 줄을 건너뛰고,
  // 곡은 영영 "덜 된" 채로 남는다. 곡 하나를 열었을 때만 덮어쓴다.
  assert.match(block, /body\.replace === true/, "곡 단위에서만 켜지는 길이어야 한다");
  assert.match(block, /lines\[job\.item\.at\] = `> \$\{next\}`/);
  // `>`가 둘 이상 쌓인 자리는 어느 것을 바꿔야 하는지 알 수 없다
  assert.match(block, /if \(\/\^\\s\*>\/\.test\(lines\[at \+ 1\] \|\| ""\)\) return \[\]/);
  // 덮어쓰기는 사람이 무엇이 바뀌는지 본 뒤에만 한다
  assert.match(block, /if \(body\.preview === true\) return Response\.json\(\{ preview: true/);
});

test("the bulk tool still never replaces", () => {
  // replace를 켜지 않으면 채우기만 한다 — 일괄 실행이 조용히 덮어쓰면 안 된다
  const bulk = fs.readFileSync(new URL("../app/admin/en-translation-gap.js", import.meta.url), "utf8");
  assert.ok(!/replace: true/.test(bulk), "일괄 화면이 덮어쓰기를 켠다");
  const perSong = fs.readFileSync(new URL("../app/admin/song-translation-finish.js", import.meta.url), "utf8");
  assert.match(perSong, /replace: true, preview: true/, "곡 화면은 미리보기부터 부른다");
});

test("the gap console counts with songNeeds, not its own rules", () => {
  // 화면이 따로 세면 도구마다 숫자가 달라진다
  assert.match(route, /if \(action === "gapOverview"\)/);
  assert.match(route, /const need = songNeeds\(song\)/);
  const page = fs.readFileSync(new URL("../app/admin/tools/page.js", import.meta.url), "utf8");
  assert.match(page, /<GapConsole \/>/);
});
