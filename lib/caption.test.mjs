...
test("a caption carries the hook, the note and the credit — and asks for nothing", () => {
  const caption = buildCaption(korean, now, [], { sets: lyricSets });
  const [hook, body, info, tags] = caption.split("\n\n");
  ...
  // 곡 정보 다음은 바로 해시태그다. 권유 문구를 뺐다 — 기록이지 유입 장치가 아니다.
  assert.match(tags, /^#/, "곡 정보 다음 블록은 해시태그여야 한다");
});
...
test("three languages each produce a complete caption under the limits", () => {
  for (const song of [korean, english, japanese]) {
    ...
    assert.ok(!/프로필 링크/.test(caption), `${song.artist}: 권유 문구가 남아 있다`);
  }
});
...
test("movie captions follow the same shape", () => {
  ...
  assert.ok(!/프로필 링크/.test(caption), "영화 캡션도 권유하지 않는다");
