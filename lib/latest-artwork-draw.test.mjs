...
import { ARTWORK_COMPOSITIONS, latestArtworkPlan } from "./latest-artwork.js";
...
test("220개 패턴 조합이 모두 오류 없이 붓질 목록을 완성한다", () => {
  let rendered = 0;
  for (const [score, shape] of Object.entries(SHAPES)) {
    const basePlan = dayFor(score, shape);
    assert.ok(basePlan, `${score} 기준 plan이 필요하다`);
    for (const composition of ARTWORK_COMPOSITIONS) {
      const plan = { ...basePlan, composition, variant: composition.index, patternId: `${score}:${composition.id}` };
      const { context, marks } = recordingContext();
      const hand = createHand({ widthUnits: W, heightUnits: H, reducedMotion: true, emit: (_a, _b, draw) => draw(context), clear() {} });
      for (const task of latestArtworkTasks({ hand, plan, W, H, random: stream(plan.seed) })) task();
      assert.ok(marks.length > 30, `${plan.patternId}의 붓질이 너무 적다`);
      rendered++;
    }
  }
  assert.equal(rendered, 220);
});
