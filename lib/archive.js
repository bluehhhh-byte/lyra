...
export function summarizeArchiveMonth(entries) {
  ...
  const themes = tally(movies.flatMap((item) => item.themes || []));
  const parts = [];
  if (emotions[0]) parts.push(`${emotions.slice(0, 2).map(([name]) => name).join("과 ")}의 음악을 자주 기록했고`);
  if (themes[0]) parts.push(`영화에서는 ${themes.slice(0, 2).map(([name]) => name).join("과 ")}의 이야기가 반복되었습니다`);
  return {
    text: parts.length ? `이번 달에는 ${parts.join(", ")}.` : "",
    emotions,
    themes,
    songs: songs.length,
