...
function validateCarouselDesign(code) {
  const rules = [
    ...
    [/layoutCarouselQualifier\(/, "긴 피처링 정보는 제목보다 작은 보조 줄로 분리해야 합니다"],
    [/layoutCarouselTitle\(/, "긴 표지 제목은 줄 수와 글자 크기를 탄력적으로 조절해야 합니다"],
    [/titleLayout\.lines\.forEach/, "계산된 표지 제목을 여러 줄로 그려야 합니다"],
    [/titleLayout\.lines\.length === 1/, "제목 줄 수에 따라 보조 정보를 재배치해야 합니다"],
  ];
  ...
  assert.doesNotMatch(cover, /title_ko/, "커버 카드에는 번역 제목을 넣으면 안 됩니다");
  assert.match(about, /title_ko/, "곡 설명 카드에는 한글 번역 제목이 필요합니다");
