...
function validateCarouselDesign(code) {
  ...
  assert.match(about, /wrap\(ctx, appearance, maxW\)\.slice\(0, 2\)/, "긴 수록 정보는 두 줄 안에서 읽을 수 있어야 합니다");
  assert.match(about, /const noteBottom = appearance \? 900 : H - 230/, "수록 정보가 있을 때 곡 설명과 작품 정보 사이에 충분한 여백이 필요합니다");
  assert.match(about, /ctx\.textAlign = "right"[\s\S]*fillText\("SONG NOTE", W - PAD, 950\)/, "SONG NOTE 표기는 설명 우측 하단의 캡션으로 보여야 합니다");
  assert.match(about, /appearanceY - 46/, "곡 설명과 수록 정보는 여유 있는 구분선 간격으로 분리되어야 합니다");
}
