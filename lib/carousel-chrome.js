...
// 발췌라는 사실만 밝히는 한 줄. 완곡으로 읽히면 안 된다.
// y는 carouselTruncatedNoteY 한 곳에서 나오고, 본문은 그만큼을 carouselBodyBox로
// 미리 비워 둔다. 여기서 본문을 밀어낼 일은 없다.
export function drawTruncatedNote(ctx, { note, align = "left" } = {}) {
  if (!note) return;
  ...
  ctx.fillText(note, x, carouselTruncatedNoteY());
  ctx.restore();
}
