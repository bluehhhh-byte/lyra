...
const overview = buildAdminOverview({
  songs: [completeSong],
  // 완결된 영화에는 감정도 있다 — 영화가 감정 궤도에 들어간 뒤로 감정 없음은 결손이다
  movies: [{ tags: ["드라마"], comment: "기록", synopsis: ["줄거리"], year: 2026, poster: "https://poster", emotion: "위로" }],
  contentStore: "neon",
...
// 폴백 경고는 현황판에서 관리자 레이아웃의 배너로 옮겼다. 현황판 것은 서버
// 렌더 시점 한 번뿐이라 화면을 열어 둔 채 장애가 시작되면 끝까지 조용했고,
// /admin 한 페이지에만 있어 다른 관리자 화면에서는 아무 경고가 없었다.
const banner = fs.readFileSync(new URL("../app/admin/fallback-banner.js", import.meta.url), "utf8");
assert.match(banner, /role="alert"/);
assert.match(banner, /파일 백업에서 읽는 중/);
assert.match(banner, /저장·등록·수정이 실패합니다/, "무엇이 막혔는지 말해야 사람이 헤매지 않는다");
assert.match(banner, /setInterval/, "한 번 그리고 마는 경고는 열어 둔 화면에서 거짓이 된다");

const layout = fs.readFileSync(new URL("../app/admin/layout.js", import.meta.url), "utf8");
assert.match(layout, /FallbackBanner/, "모든 관리자 화면에 걸려야 한다");

const component = fs.readFileSync(new URL("../app/admin/admin-overview.js", import.meta.url), "utf8");
assert.doesNotMatch(component, /role="alert"/, "같은 경고를 두 번 띄우지 않는다");
