...
// GitHub이 대답하지 않으면 서버리스 함수가 최대 실행 시간까지 붙들려 있고,
// 그 시간이 통째로 무료 한도에서 나간다. 10초면 정상 응답에는 넉넉하고
// 매달린 연결은 끊는다.
...
async function invalidate(kind, slug, { removed = false } = {}) {
  ...
  if (kind === "song") revalidatePath("/songs/motifs");
  revalidatePath("/");

  // 삭제는 다르다. 지운 곡이 실려 있던 목록이 어디까지인지 우리가 모른다 —
  // 홈, 아카이브, 통계, 취향, 태그별 페이지, 같은 앨범 동반곡…. 태그 무효화는
  // 데이터 캐시만 비우므로, 이미 만들어진 HTML은 최대 6시간 그 곡을 계속 보여 준다.
  // 실제로 Mr. Big <Take Cover>를 지운 뒤 상세 페이지와 검색은 즉시 비었는데
  // 홈만 2.4시간 된 HTML(Age 8635)을 계속 내놓았다.
  //
  // "layout"은 루트 아래 전부를 비운다. 저장할 때마다 그러면 626개 페이지가
  // 한꺼번에 다시 렌더돼 Neon을 다시 태우므로, 삭제일 때만 쓴다 — 삭제는 드물고,
  // 지운 것이 남아 보이는 쪽이 훨씬 나쁘다.
  if (removed) revalidatePath("/", "layout");
}
...
function makeStore(dir, kind) {
  ...
  const remove = async (slug, message) => {
    ...
    if (databaseContentEnabled()) {
      ...
      await invalidate(kind, safeSlug(slug), { removed: true });
      return;
    }
    if (useGit) {
...
export async function writeData(name, content, message) {
  ...
  if (useGit) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const cur = await fetch(url, { headers: head, cache: "no-store", signal: AbortSignal.timeout(GITHUB_TIMEOUT_MS) });
...
export async function commitFiles(files, message) {
  ...
  const call = async (url, init) => {
    const res = await fetch(url, { ...init, headers: head, cache: "no-store", signal: AbortSignal.timeout(GITHUB_TIMEOUT_MS) });
    if (!res.ok) {
...
const hasDataBackup = (name) =>
  fs.existsSync(path.join(process.cwd(), "data", path.basename(name)));
...
export async function readRuntimeData(name, fallback) {
  ...
  if (!row) {
    if (!hasDataBackup(name)) return fallback;
    noteContentFallback();
    console.error(`[store] ${name} DB 행 없음 — 파일 백업으로 서빙한다`);
    return readData(name, fallback);
  }
  try {
    ...
    noteContentFallback();
    console.error(
      `[store] ${name} DB JSON 파싱 실패 — ${hasDataBackup(name) ? "파일 백업" : "기본값"}으로 서빙한다: ${err.message}`
    );
    return readData(name, fallback);
  }
}
