...
function assemble({ credit, tags, stamp }) {
  return [credit && `| ${credit}`, tags && tags.map((tag) => `#${tag}`).join(" "), stamp && `(${stamp})`]
    .filter(Boolean)
    .join("\n\n");
}
...
export function buildCaption(song = {}, now = new Date(), extraTags = []) {
  return assemble({
    ...
    tags: suggestHashtags(song, { extra: extraTags }),
...
export function buildMovieCaption(movie = {}, now = new Date(), extraTags = []) {
  return assemble({
    ...
    tags: suggestHashtags(
      { artist: movie.director, title: movie.title },
      { extra: extraTags, fixed: FIXED_MOVIE_TAGS },
    ),
    stamp: readableStamp(now),
  });
}
...
export function buildMovieCarouselCaption(preset = {}, now = new Date(), extraTags = []) {
  return assemble({
    ...
    tags: suggestHashtags({ artist: "Cyno" }, { extra: extraTags, fixed: FIXED_MOVIE_TAGS }),
    stamp: readableStamp(now),
  });
}
