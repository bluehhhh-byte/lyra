// 인스타그램 캡션.
//
// 예전에는 한 줄이었다 — `| 가수 - 제목 (년도) #가수 #가사 #260714_1531`.
// 사이트는 곡마다 해설·감정·키워드·"이럴 때 듣는다" 한 줄을 다 갖고 있는데
// 게시물에는 그중 아무것도 나가지 않았다. 캡션은 인스타그램이 색인하는
// 유일한 텍스트라, 그 자산을 안 흘려보내면 발견될 길이 없다.
//
// 타임스탬프도 해시태그였다(#260714_1531). 아무도 검색하지 않는 태그가
// 30개 예산에서 한 자리를 차지하고 있었다 — 이제 본문 괄호로 내린다.
// 들어오는 캡션을 파싱하는 extractBody(lib/admin/instagram.js)는 기존
// 게시물을 가져올 때만 쓰이므로 이 변경과 무관하다(2026-09-12 확인).
export const INSTAGRAM_CAPTION_LIMIT = 2200;
export const INSTAGRAM_HASHTAG_LIMIT = 30;

// 캡션 본문에 싣는 해설 길이. 인스타그램은 두 줄쯤에서 "더 보기"로 접으므로
// 그 앞에서 할 말이 끝나야 한다.
const COMMENT_MAX_SENTENCES = 3;
const COMMENT_MAX_CHARS = 240;
...
// 문장 경계에서 자른다. 낱말 중간에서 끊긴 해설은 미완성으로 읽힌다.
export function clipComment(value, { maxSentences = COMMENT_MAX_SENTENCES, maxChars = COMMENT_MAX_CHARS } = {}) {
  ...
  out = out.trim();
  // 부호 없이 길게 이어 쓴 해설은 위 루프가 한 문장으로 보고 통째로 가져온다.
  // 그대로 두면 캡션이 수백 자로 불어나 인스타그램이 "더 보기"로 접는다.
  if (!out || out.length > maxChars) {
    const cut = text.slice(0, maxChars);
    const lastSpace = cut.lastIndexOf(" ");
    // 낱말 중간에서 끊지 않는다 — 띄어쓰기가 없는 문장이면 어쩔 수 없이 글자로.
    out = (lastSpace > maxChars * 0.6 ? cut.slice(0, lastSpace) : cut).trim();
  }
  // 문장 부호 없이 잘렸으면 말줄임으로 끝을 알린다
  return /[.!?。]$/.test(out) || out === text ? out : `${out}…`;
}
...
// ── 캡션 조립 ────────────────────────────────────────────────────────────
// 한때 "전체 가사와 해설 — 프로필 링크" 한 줄이 여기 있었다. 카드에서 권유
// 문구를 뺀 것과 같은 이유로 캡션에서도 뺐다 — 기록이지 유입 장치가 아니다.
// 프로필 링크는 계정 상단에 이미 있고, 찾을 사람은 찾는다.

// 빈 값이면 그 줄 자체가 없다. 빈 줄을 남기면 "쓰다 만 글"로 보인다.
function assemble({ hook, body, info, tags, stamp }) {
  const blocks = [hook, body, info].filter(Boolean);
  const lines = blocks.join("\n\n");
  const tail = [tags && tags.map((tag) => `#${tag}`).join(" "), stamp && `(${stamp})`]
    .filter(Boolean)
    .join("\n\n");
...
export function buildCaption(song = {}, now = new Date(), extraTags = [], { sets = [] } = {}) {
  const head = creditLine(song.artist, song.title, song.year);
  const translated = song.lang && song.lang !== "ko" ? " · 번역 포함" : "";
  return assemble({
    hook: String(song.listen_when || "").trim(),
    body: clipComment(song.comment),
    info: `${head}${translated}`,
...
export function buildMovieCaption(movie = {}, now = new Date(), extraTags = [], { sets = [] } = {}) {
  const head = creditLine(movie.director, movie.title, movie.year);
  // 영화의 훅은 감상 코멘트의 첫 문장이다 — listen_when에 해당하는 칸이 없다.
  const hook = firstSentence(movie.comment);
  const body = clipComment(movie.comment, { maxSentences: 3 });
  return assemble({
    hook,
    // 훅이 곧 해설의 첫 문장이라 그대로 두면 같은 말이 두 번 나온다
    body: body === hook ? "" : body,
    info: head,
...
export function buildMovieCarouselCaption(preset = {}, now = new Date(), extraTags = [], { sets = [] } = {}) {
  return assemble({
    hook: String(preset.headline || "").trim(),
    body: clipComment(preset.note || preset.description),
    info: "",
