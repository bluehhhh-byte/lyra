...
// ── 캡션 조립 ────────────────────────────────────────────────────────────
const CTA_SONG = "전체 가사와 해설 — 프로필 링크";
const CTA_MOVIE = "전체 감상 — 프로필 링크";

// 빈 값이면 그 줄 자체가 없다. 빈 줄을 남기면 "쓰다 만 글"로 보인다.
function assemble({ hook, body, info, cta, tags, stamp }) {
  const blocks = [hook, body, info, cta].filter(Boolean);
