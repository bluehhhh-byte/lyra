...
assert.match(queue, /Promise\.all\(\[getAllMoviesRuntime\(\), getWatchedRuntime\(\)\]\)/, "감상 글과 왓챠 기록을 읽기 전용으로 함께 조합해야 한다");
...
assert.doesNotMatch(queue, /gemini|writeRuntimeData|save|database|\.json/i, "후보 화면은 AI나 새 상태 저장소를 사용하지 않는다");
