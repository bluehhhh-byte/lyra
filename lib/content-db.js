...
// BOM(U+FEFF), 줄 끝 공백, 따옴표. 2026-08-19까지 이틀 동안 Neon 저장소가 꺼져 있었는데,
// 값이 "neon"이 아니라 "<BOM>neon"이었기 때문이다(PowerShell Out-File이 BOM을 붙인다).
// 화면에는 neon이라고 보이니 아무도 의심하지 않았다. 값을 다듬어 비교한다.
const flag = (v) =>
  String(v ?? "")
    .replace(/^\uFEFF/, "")
    .replace(/^["']|["']$/g, "")
...
export function databaseContentEnabled(env = process.env) {
  return flag(env.LYRA_CONTENT_STORE) === "neon";
}
