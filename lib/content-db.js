...
export const CACHE_WARNING_BYTES = CACHE_LIMIT * 0.9;

export function cachePayloadWarning(bytes, limit = CACHE_LIMIT) {
  if (bytes <= limit * 0.9) return "";
  return `[content-db] 캐시 payload ${(bytes / 1048576).toFixed(2)}MB — ${(limit / 1048576).toFixed(0)}MB 한도에 근접. ` +
    `넘으면 캐시가 통째로 무효가 되어 요청마다 DB를 다시 읽는다.`;
}

// 지금까지 실제로 만들어 본 payload 중 가장 큰 것. 경고를 console에만 찍으면
// Vercel 로그를 뒤지는 사람에게만 보인다 — 즉 아무에게도 안 보인다.
// /api/version이 이 값을 읽어 "한도까지 몇 %"를 밖으로 내놓는다.
...
export const packRows = (rows) => {
  ...
  if (packed.length > peakPayloadBytes) peakPayloadBytes = packed.length;
  // 곡이 늘면 압축본도 언젠가 한도를 넘는다. 그날 다시 조용히 느려지지 않도록 미리 말한다.
  const warning = cachePayloadWarning(packed.length);
  if (warning) console.warn(warning);
  return packed;
};

// 이 인스턴스가 관측한 최대 캐시 항목 크기. 아직 아무것도 압축하지 않았으면
// bytes가 0이고 measured가 false다 — "안전"이 아니라 "모른다"는 뜻이다.
export function cachePayloadPeak(bytes = peakPayloadBytes, limit = CACHE_LIMIT) {
  return {
    measured: bytes > 0,
    bytes,
    limit,
    percent: Math.round((bytes / limit) * 1000) / 10,
    nearLimit: bytes > limit * 0.9,
    overLimit: bytes > limit,
  };
}

// 꺼낼 때도 잰다. packRows는 캐시 미스에만 도는데, 정상 운영에서는 대부분 적중이라
// 그것만 보면 크기를 아는 인스턴스가 사실상 없다. 꺼내는 값이 곧 저장된 값이다.
export const unpackRows = (packed) => {
  if (packed.length > peakPayloadBytes) peakPayloadBytes = packed.length;
  return JSON.parse(zlib.gunzipSync(Buffer.from(packed, "base64")).toString());
};

// 아직 아무것도 읽지 않은 인스턴스를 위한 확인용 조회. 메타 항목 하나만 꺼내
// 길이를 재고 압축은 풀지 않는다 — Data Cache 적중이면 Neon까지 가지 않는다.
export async function cachePayloadProbe(kind = "song") {
  if (peakPayloadBytes > 0) return cachePayloadPeak();
  try {
    const packed = await metaEntry(kind)();
    if (packed && packed.length > peakPayloadBytes) peakPayloadBytes = packed.length;
  } catch {
    // 크기를 못 쟀다고 /api/version이 죽으면 안 된다. measured:false로 남는다.
  }
  return cachePayloadPeak();
}
...
// 한도를 넘긴 항목만 담는 프로세스 안 그물.
//
// 한도를 넘기면 Data Cache가 저장을 건너뛰므로 모든 요청이 Neon까지 내려간다 —
// 2026-08-22에 월 전송 한도를 태운 것이 정확히 이 경로다. 그 상태에 다시 빠져도
// 인스턴스당 한 번만 읽도록 여기서 한 겹 더 잡는다. 샤드를 늘리기 전까지 버티는
// 임시 그물이지 대책이 아니다 — overLimit이 뜨면 CONTENT_SHARDS를 올려야 한다.
//
// revision으로 무효화한다. 태그 무효화는 이 Map에 닿지 못하므로 저장 직후 최대
// REVISION_TTL_MS 동안 옛 값이 보일 수 있다 — 검색 인덱스(lib/search-index.js)가
// 쓰는 것과 같은 방식이고, 애초에 캐시가 고장난 상태에서만 켜진다.
// ponytail: 넘친 항목만 들고 있으므로 최대 (샤드 4 + 메타 1) × kind 2 = 10개.
const oversizedMemo = new Map();
const REVISION_TTL_MS = 60_000;
let memoRevision = "";
let memoCheckedAt = 0;

async function memoRevisionCached() {
  if (memoCheckedAt && Date.now() - memoCheckedAt < REVISION_TTL_MS) return memoRevision;
  memoRevision = await contentRevision();
  memoCheckedAt = Date.now();
  return memoRevision;
}

// 내부용이지만 테스트가 직접 몰아볼 수 있게 내보낸다 — 이 분기는 캐시가 이미
// 고장난 뒤에만 실행되므로 실제 운영에서 관찰될 기회가 거의 없다.
export async function packedThroughCache(key, entry) {
  const hit = oversizedMemo.get(key);
  if (hit && hit.revision === (await memoRevisionCached())) return hit.packed;
  const packed = await entry();
  if (packed && packed.length > CACHE_LIMIT)
    oversizedMemo.set(key, { revision: await memoRevisionCached(), packed });
  else oversizedMemo.delete(key);
  return packed;
}
...
export async function listCachedContentRows(kind) {
  const shards = await Promise.all(
    Array.from({ length: CONTENT_SHARDS }, (_, i) =>
      packedThroughCache(`${kind}:shard${i}`, shardEntry(kind, i)))
  );
...
export async function listCachedContentMeta(kind) {
  return unpackRows(await packedThroughCache(`${kind}:meta`, metaEntry(kind)));
}
