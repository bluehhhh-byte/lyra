...
const feelsMain = layoutCarouselTitle(feels.main, measure, 888, 2);
const feelsCredit = layoutCarouselQualifier(feels.qualifier, measure, 888);
assert.deepEqual(feelsMain.lines, ["Feels"]);
assert.ok(feelsCredit.lines.length <= 2);
assert.ok(feelsCredit.lines.every((line) => measure(line, feelsCredit.fontSize) <= 888));
