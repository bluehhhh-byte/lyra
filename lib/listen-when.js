...
//
// 이전엔 고정 문자열 하나가 '~때'/'~밤'/'~날'/'~길'/'~새벽' 다섯 개를 예로 들었는데,
// 다섯 중에서 고르라 해도 실제로는 밤에 쏠렸다(987곡 중 397곡). 예시를 나열하는 것만으로는
// 분포를 못 바꾼다 — 매 호출마다 자바스크립트가 직접 각도 하나를 무작위로 골라 그 각도만
// 지시해야 시간대 아닌 각도도 실제로 나온다.
const LISTEN_WHEN_ANGLES = [
  "하루 중 한 시간대를 골라 그 시간에 어울리는 장면 하나를 그려라. '~새벽'/'~아침'/'~낮'/'~오후'/'~저녁'/'~밤' 중 하나로 끝내되, 밤으로 쏠리지 말고 여섯 시간대를 고르게 써라.",
  "곡을 들을 법한 구체적인 행동이나 활동 하나를 그려 '~때'로 끝내라 — 설거지할 때, 혼자 운전할 때, 이어폰을 낄 때 같은 종류.",
  "장소나 이동하는 동작을 하나 그려 '~길'/'~곳'/'~자리' 같은 명사로 끝내라.",
  "날씨나 계절의 한 장면을 그려 '~날'로 끝내라 — 비, 눈, 바람, 첫눈, 장마 같은 종류.",
  "마음이 움직이는 순간 하나를 그려 '~때'/'~순간'으로 끝내라 — 시간대나 날씨를 끌어오지 마라.",
  "관계에서 벌어지는 구체적 장면 하나를 그려 '~날'/'~때'로 끝내라 — 이별 직후, 재회를 기다릴 때 같은 종류.",
];

export function LISTEN_WHEN_RULE() {
  const angle = LISTEN_WHEN_ANGLES[Math.floor(Math.random() * LISTEN_WHEN_ANGLES.length)];
  return `listenWhen: 이 곡을 듣기 좋은 순간을 그리는 장면 한 줄, 12~22자. ${angle} 구체적 상황 하나만 담을 것. 서술어(~다/~요)로 끝내지 말 것. 문장부호·따옴표·해시태그 금지. 가사의 정서에 근거하되 가사 문장을 그대로 옮기지 마라`;
}
