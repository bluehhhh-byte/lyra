...
// SVG text의 좌우 경계 — anchor에 따라 x가 왼쪽/가운데/오른쪽 어디인지 달라진다
export function textBounds(x, anchor, w) {
  if (anchor === "start") return [x, x + w];
  if (anchor === "end") return [x - w, x];
  return [x - w / 2, x + w / 2];
}

// 라벨을 viewBox 안으로 민다. 글자가 잘리느니 자리를 옮긴다 — 크기를 줄이지 않는다.
// pad는 stroke 배경(paint-order)이 먹는 두께까지 감안한 안전 여백이다.
export function clampLabel({ x, y, anchor, w, h }, { width, height, pad = 4 }) {
  const [l, r] = textBounds(x, anchor, w);
  let dx = 0;
  if (l < pad) dx = pad - l;
  else if (r > width - pad) dx = width - pad - r;
  // y는 baseline이다. 위로는 글자 높이만큼, 아래로는 descender만큼 여유가 필요하다
  const top = y - h * 0.8;
  const bottom = y + h * 0.25;
  let dy = 0;
  if (top < pad) dy = pad - top;
  else if (bottom > height - pad) dy = height - pad - bottom;
