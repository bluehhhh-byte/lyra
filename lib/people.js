...
import { emotionCenter, moodType, INSUFFICIENT_SAMPLE_LABEL, MOOD_MIN_SAMPLE } from "./emotion-model.js";
...
// 아티스트를 성격처럼 판정하지 않는다. 이 함수가 설명하는 대상은 해당 아티스트의
// 곡과 함께 남긴 '기록'이며, 감정이 있는 곡이 3곡 미만이면 해석을 유보한다.
export function artistEmotionProfiles(songs) {
  ...
  return [...groups.entries()].map(([artist, artistSongs]) => {
    const center = emotionCenter(artistSongs.map((song) => song.emotion));
    const sample = center?.n || 0;
    const deferred = sample < MOOD_MIN_SAMPLE;
    const type = deferred ? INSUFFICIENT_SAMPLE_LABEL : moodType(center);
    return {
      artist,
      ...
      deferred,
      summary: deferred
        ? `감정이 기록된 곡이 ${sample}곡이라 이 기록의 흐름을 단정하지 않는다.`
        : `이 아티스트의 곡과 함께 남긴 기록은 '${type}' 영역에 모였다.`,
    };
  }).sort((a, b) => b.sample - a.sample || b.songs - a.songs || a.artist.localeCompare(b.artist, "ko"));
}
...
export function getPeopleSummaries(films = allFilms()) {
  return getAllPeople(films).map((person) => ({
    ...
    actorAverageRating: (() => {
      const rated = person.acted.filter((work) => work.rating != null);
      return rated.length ? rated.reduce((sum, work) => sum + Number(work.rating), 0) / rated.length : null;
    })(),
