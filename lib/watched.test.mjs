...
import { countryDistribution, directorOtherWorks, directorPreferences, frequentActors, genreRatingCross, getWatched, ratingDistribution, rewatchGroups, runtimeInsights, watchedTimeline, yearlyRatingTrend } from "./watched.js";
...
const directorWorks = directorOtherWorks([
  { code: "current", title: "현재", year: 2020, tmdbId: 1, director_ko: "감독", rating: 5 },
  { code: "other-low", title: "다른 작품 1", year: 2021, tmdbId: 2, director_ko: "감독", rating: 3 },
  { code: "other-high", title: "다른 작품 2", year: 2019, tmdbId: 3, director_ko: "감독", rating: 4.5 },
  { code: "other-director", title: "남의 작품", year: 2022, tmdbId: 4, director_ko: "다른 감독", rating: 5 },
], { title: "현재", year: 2020, tmdbId: 1, director_ko: "감독" });
assert.deepEqual(directorWorks.map((movie) => movie.key), ["other-high", "other-low"]);
assert.ok(directorWorks.every((movie) => movie.rating != null));
console.log("✓ 영화 상세용 감독의 다른 관람 기록을 현재 작품 없이 별점순으로 계산");

const timeline = watchedTimeline([
  { year: 1999, watchedAt: "2024-01-03" },
  { year: "2000", watched_at: "2024-01-28" },
  { year: 2000, recordedAt: "2024-03-01" },
  { year: "미상", date: "" },
]);
assert.deepEqual(timeline.releaseYears, [{ period: 1999, count: 1 }, { period: 2000, count: 2 }]);
assert.deepEqual(timeline.viewingYears, [{ period: "2024", count: 3 }]);
assert.deepEqual(timeline.viewingMonths, [{ period: "2024-01", count: 2 }, { period: "2024-03", count: 1 }]);
assert.equal(timeline.missingDates, 1);
assert.equal(watchedTimeline(all).missingDates, all.length, "현재 왓챠 원본에는 관람일이 없다");
console.log("✓ 작품 연도와 실제 관람일 연·월 밀도를 분리하고 누락을 센다");
