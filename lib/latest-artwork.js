...
// 다섯 가지 화면 배치와 네 가지 운동축을 교차해, 각 지시문이 같은 구도로만
// 되풀이되지 않게 한다. 11개 지시문 × 20개 조형 문법 = 220개 고유 패턴이다.
const LAYOUTS = [
  { id: "field", x: 0.5, y: 0.5, width: 0.94, height: 0.88, density: 1 },
  { id: "west", x: 0.32, y: 0.5, width: 0.58, height: 0.82, density: 1.15 },
  { id: "east", x: 0.68, y: 0.5, width: 0.58, height: 0.82, density: 0.88 },
  { id: "crown", x: 0.5, y: 0.31, width: 0.84, height: 0.54, density: 1.28 },
  { id: "root", x: 0.5, y: 0.69, width: 0.84, height: 0.54, density: 0.76 },
];

const AXES = [
  { id: "horizon", angle: 0, width: 1, height: 0.78 },
  { id: "column", angle: Math.PI / 2, width: 0.72, height: 1 },
  { id: "ascent", angle: -Math.PI / 4, width: 0.88, height: 0.88 },
  { id: "descent", angle: Math.PI / 4, width: 0.88, height: 0.88 },
];

export const ARTWORK_COMPOSITIONS = Object.freeze(LAYOUTS.flatMap((layout, layoutIndex) =>
  AXES.map((axis, axisIndex) => Object.freeze({
    id: `${layout.id}-${axis.id}`,
    index: layoutIndex * AXES.length + axisIndex,
    layoutIndex,
    axisIndex,
    x: layout.x,
    y: layout.y,
    width: layout.width * axis.width,
    height: layout.height * axis.height,
    density: layout.density,
    angle: axis.angle,
  })),
));

export const ARTWORK_PATTERN_COUNT = SCORES.length * ARTWORK_COMPOSITIONS.length;
export const artworkPatternCatalog = () => SCORES.flatMap((score) =>
  ARTWORK_COMPOSITIONS.map((composition) => `${score.id}:${composition.id}`),
);
...
export function latestArtworkPlan(latest = {}) {
  ...
  const avoidedPatterns = new Set((latest.artworkPatternAvoid || []).filter(Boolean));
  const freshCompositions = ARTWORK_COMPOSITIONS.filter((composition) => !avoidedPatterns.has(`${chosen.id}:${composition.id}`));
  const compositionCandidates = freshCompositions.length ? freshCompositions : ARTWORK_COMPOSITIONS;
  const composition = compositionCandidates[(seed >>> 8) % compositionCandidates.length];
  const patternId = `${chosen.id}:${composition.id}`;
