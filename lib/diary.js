import { getAllSongs, getAllSongsRuntime } from "./songs";
...
export function getDiary(songs = getAllSongs()) {
  const byDay = new Map();
  for (const s of songs) {
    const day = kstDay(s.published || s.date);
    if (!day) continue;
...
export async function getDiaryRuntime() {
  return getDiary(await getAllSongsRuntime());
}
