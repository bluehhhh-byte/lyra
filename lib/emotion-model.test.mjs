...
import {
  ...
  monthCharacterLabel, INSUFFICIENT_SAMPLE_LABEL, MOOD_NEUTRAL_BAND, MOOD_MIN_SAMPLE, MONTH_CHARACTER_THRESHOLD,
...
// 정서 유형 경계값
{
  ...
  // 표본이 적으면 단정하지 않는다
  assert.equal(INSUFFICIENT_SAMPLE_LABEL, "판단 유보");
  assert.equal(moodType({ v: 3, a: 3, n: MOOD_MIN_SAMPLE - 1 }), INSUFFICIENT_SAMPLE_LABEL);
  assert.equal(moodType(null), INSUFFICIENT_SAMPLE_LABEL);
}
