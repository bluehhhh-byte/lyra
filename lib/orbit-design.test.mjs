...
const scaleToggle = await readFile(new URL("../app/archive/orbit-scale-toggle.js", import.meta.url), "utf8");

// 척도 — 연도 비교
assert.match(orbit, /const DOMAIN = \[-3, 3\]/, "연도별 비교를 위해 축은 고정 척도여야 한다");
assert.match(orbit, /focusDomains[\s\S]*axisRange\(points/, "미세한 움직임을 읽는 명시적 확대 보기가 있어야 한다");
assert.match(scaleToggle, /전체 척도/, "연도 간 비교용 고정 척도로 돌아갈 수 있어야 한다");
assert.match(scaleToggle, /움직임 확대/, "중앙에 모인 월별 이동을 확대해서 볼 수 있어야 한다");
...
// 읽혔다. 색을 시간에 내주면 궤적이 색만으로 읽힌다.
{
  ...
  assert.match(orbit, /색 · 시간 순서/, "색이 무엇을 뜻하는지 화면에 적어야 한다 — 범례 없는 색은 장식이다");
}

// 포커스 — 열두 달을 같은 강도로 보이면 현재 달을 찾기 어렵다.
{
  assert.match(orbit, /const nearActive = i === activeIndex \|\| i \+ 1 === activeIndex/, "선택 월과 맞닿은 이동 구간을 찾아야 한다");
  assert.match(orbit, /nearActive \? 0\.88 : 0\.14/, "확대 보기에서는 선택 월 주변 이동만 선명해야 한다");
  assert.match(orbit, /const showLabel = nearActive \|\| s\.turningPoint/, "월 라벨은 선택 주변과 전환점 중심으로 줄여야 한다");
...
{
  const move = orbit.match(/focused \? \(nearActive \? ([\d.]+) : ([\d.]+)\) : \(nearActive \? ([\d.]+) : ([\d.]+)\)/);
  assert.ok(move, "포커스 상태별 이동선 opacity를 찾지 못했다");
  const [, focusActive, focusRest, fullActive, fullRest] = move.map(Number);
  assert.ok(focusActive > focusRest, "확대 보기에서 선택 월 주변 이동이 더 선명해야 한다");
  assert.ok(fullActive > fullRest, "전체 보기에서도 선택 월 주변 이동이 더 선명해야 한다");
  assert.ok(focusRest < 0.5 && fullRest < 0.5, "나머지 이동선은 데이터 점보다 뒤에 있어야 한다");
}
