...
assert.match(comparison, /data-series="base"[\s\S]*strokeDasharray="9 6"/, "기준 월은 점선이어야 한다");
assert.match(comparison, /data-series="compare"[\s\S]*stroke="var\(--color-accent\)"/, "비교 월은 강조 실선이어야 한다");
...
assert.match(comparison, /const AXES = \["각성", "밝기", "다양성", "기록 밀도", "전월 이동"\]/);
assert.match(comparison, /function profiles\(/);
...
assert.match(comparison, /const FLOOR = 0\.25/);
assert.match(comparison, /Math\.pow/);
