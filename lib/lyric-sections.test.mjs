...
import { applySections, originalLines, tightenStanzas } from "./admin/lyric-sections.js";
...
// 줄마다 빈 줄이 끼어 한 줄이 곧 한 연이 되어 버린 가사 — 다른 곡들은 연 안에서
// 줄이 붙어 있고 빈 줄은 연 경계에만 있다. tighten은 그 형태로 되돌린다.
{
  const loose = [
    "첫 줄이다",
    "> first",
  ...
  const out = applySections(loose, [{ at: 0, section: "Verse 1" }, { at: 2, section: "Chorus" }], { tighten: true });
  assert.equal(out, [
    "[Verse 1]",
    "첫 줄이다",
    "> first",
  ...
  // 기본값은 손대지 않는다 — 빈 줄로 연을 나눈 곡을 뭉개면 안 된다
  const untouched = applySections(loose, [{ at: 0, section: "Verse 1" }]);
  assert.ok(untouched.includes("> first\n\n둘째"), "tighten 없이는 빈 줄이 남는다");
}

console.log("✓ 가사 구간 — 연 붙이기");
