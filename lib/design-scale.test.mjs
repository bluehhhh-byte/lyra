...
assert.match(browse, /\{s\.emotion && <span className="[^"]*">\{s\.emotion\}<\/span>\}/, "감정 낱말도 함께 보여야 색만으로 못 읽는 사람에게 남는다");

// 아티스트명·연도·감정이 한 줄에 갇히면 이름이 긴 곡은 말줄임에 잘리고, 잘린
// 자리 뒤의 연도와 감정까지 같이 사라진다. 카드에서 읽어야 할 세 가지가 이름
// 길이 하나로 결정되면 안 된다 — 몇 줄이 되든 전부 보인다.
const metaLine = browse.slice(browse.indexOf('<p className="mt-0.5'), browse.indexOf("<Snippet song={s}"));
assert.ok(metaLine, "곡 카드의 메타 줄을 찾지 못했다");
assert.doesNotMatch(metaLine, /truncate|line-clamp/, "아티스트·연도·감정 줄을 잘라내면 안 된다");
