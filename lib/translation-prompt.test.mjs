...
test("두 번역 프롬프트 모두 혼용 줄을 교차 번역하라고 지시한다", async () => {
  ...
  // Rules 목록에도 옛 규칙이 남아 있으면 같은 프롬프트 안에서 정반대 지시가 된다.
  assert.doesNotMatch(songMeta, /follows its dominant language/);
  assert.doesNotMatch(songMeta, /including its English words/);
