...
assert.deepEqual(
  toAdminSong({
    title_ko: "아무것도 없어", artist_ko: "와스레란네에요", album: "싱글",
    tags: ["J-Rock"], search_aliases: ["난모네"], listen_when: "아무것도 없어도 내지르고 싶은 밤",
  }),
  {
    slug: "", title: "(제목 없음)", title_ko: "아무것도 없어",
    artist: "(아티스트 없음)", artist_ko: "와스레란네에요", album: "싱글",
    tags: ["J-Rock"], searchAliases: ["난모네"], artwork: "", comment: "",
    listenWhen: "아무것도 없어도 내지르고 싶은 밤", hasTranslation: false,
  }
);
...
// 목록이 쓰는 값은 어댑터를 통해서만 온다 — 어댑터에서 빠지면 화면에도 안 나온다
assert.match(tools, /st\.listenWhen \?\? s\.listenWhen/, "등록곡 목록이 listen_when을 보여야 한다 — 방금 바뀐 값이 있으면 그쪽을");
assert.match(tools, /st\.previousListenWhen &&/, "덮인 옛 문구도 함께 보여야 한다");
assert.match(tools, /href=\{`\/songs\/\$\{s\.slug\}`\}/, "표지를 누르면 그 곡의 글로 가야 한다");
