...
export const normalizeNewlines = (raw) => String(raw ?? "").replace(/\r\n?/g, "\n");
...
// `[...]`는 보통 구간 라벨([Chorus], [Verse 2], 화자 이름)이지만, 가사 한 줄을
// 통째로 대괄호에 넣어 둔 캡션도 있다([人はいったいどこから來て…]). 그걸 라벨로
// 삼으면 그 줄이 가사에서 사라지고, 아래 붙은 번역은 붙일 원문을 잃는다.
// 라벨은 짧고 문장이 아니다 — 긴 줄이나 CJK 문장은 가사로 본다.
const SECTION_WORDS =
  "verse|chorus|pre-?chorus|post-?chorus|bridge|intro|outro|hook|refrain|interlude|drop|solo|breakdown|후렴|벌스|브릿지|인트로|아웃트로|간주|훅";
// 구간 라벨의 꼴: 구간 낱말 + (번호) + (콜론·대시 뒤에 부른 사람).
// 부른 사람이 둘이면 길이가 금세 24자를 넘는다 — Post Malone의 <Take What You Want>는
// `[Chorus: Ozzy Osbourne & Post Malone]`(35자)이 길이 상한에 먼저 걸려 가사 한 줄로
// 새어 들어갔고, 그 줄이 "번역 필요"로 대기열에 올라 있었다. 길이가 아니라 꼴로 본다.
const SECTION_LABEL = new RegExp(`^(?:${SECTION_WORDS})(?:\\s*\\d+)?(?:\\s*[:：·\\-–—]\\s*\\S.*)?$`, "iu");

export function isSectionLabel(inner) {
  const t = (inner || "").trim();
  if (!t) return false;
  if (SECTION_LABEL.test(t)) return true;
  // 아래는 구간 낱말이 없는 것들 — 화자 이름("Rumi")이나 짧은 표시.
  // 여기서는 길이가 판단 근거다: 대괄호에 통째로 넣어 둔 가사 줄과 갈라야 한다.
  if (t.length > 24) return false;
  if (/[。、！？…]/.test(t)) return false;                    // 문장부호가 있으면 가사
  if (/[぀-ヿ㐀-鿿]/.test(t)) return t.length <= 8;           // 짧은 CJK만 화자 이름으로
  return t.split(/\s+/).length <= 3;                         // "Rumi", "Jinu" 같은 것
}
...
export function parseLyrics(body) {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (!line.trim()) {
    ...
    } else if (line.startsWith("[") && line.endsWith("]") && isSectionLabel(line.slice(1, -1))) {
      flush();
      ...
      for (let t = 0; t < k; t++) {
        ...
        // 자리가 모자라면 마지막 줄에 이어 붙인다. 예전에는 덮어써서, 원문보다 번역이
        // 많은 문단(원문 두 줄을 세 줄로 옮긴 곳)에서 앞 번역이 화면에서 사라졌다.
        if (!take.length && l.ko) { l.ko += " " + text; continue; }
        l.ko = text;
        if (span > 1) {
          l.koSpan = span;
...
// 원문 없이 번역만 남은 줄은 바로 앞 번역의 뒷부분이다 — 이어 붙인다.
// 화면에 번역만 동동 떠 있으면 어느 구절인지 알 수 없다.
function mergeOrphanTranslations(stanzas) {
  let prev = null; // 직전에 나온, 번역이 붙은 줄
  for (const st of stanzas) {
    const keep = [];
    for (const l of st.lines) {
      if (!l.en?.trim() && l.ko?.trim() && prev) {
        prev.ko = `${prev.ko} ${l.ko}`.trim();
        continue; // 이 줄은 화면에서 사라지고 앞줄 번역이 길어진다
      }
      keep.push(l);
      if (l.ko?.trim()) prev = l;
    }
...
function splitInline(stanzas) {
  ...
  return mergeOrphanTranslations(stanzas);
}
...
function readAllSongs() {
  ...
  return fs
    ...
    .map((f) => {
      ...
      return parseSongRecord(slug, raw);
    })
    .filter((song) => !song.duplicate_of)
...
function parseSongRecord(slug, raw) {
  const { meta, body } = parseFrontmatter(raw);
  return { slug, ...meta, tags: meta.tags || [], stanzas: parseLyrics(body) };
}

// DB가 진실 공급원이지만, DB가 대답하지 못할 때 사이트까지 같이 죽을 이유는 없다.
// songs/*.md는 저장소에 함께 배포되는 백업본이라 언제나 손이 닿는다.
// (2026-08-22 Neon 전송 한도 초과로 모든 읽기가 402를 냈고, 사이트가 아니라
//  빌드와 관리자 페이지가 먼저 멈췄다 — 빌드가 sitemap을 만들다 죽었기 때문이다.)
export async function getAllSongsRuntime() {
  if (!databaseContentEnabled()) return getAllSongs();
  try {
    const rows = await listCachedContentRows("song");
    return rows
      .map((row) => parseSongRecord(row.slug, row.raw))
      .filter((song) => !song.duplicate_of)
      .sort((a, b) => (b.published || b.date || "").localeCompare(a.published || a.date || ""));
  } catch (err) {
    noteContentFallback();
    console.error(`[songs] DB 읽기·파싱 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllSongs();
  }
}

// 목록용 — 가사를 뺀 곡 메타. 홈·태그·아카이브·인물·연관 추천이 쓴다.
//
// 이 화면들은 가사를 한 줄도 읽지 않는데 전곡 본문을 파싱하고 있었다. 파싱 시간의
// 절반이 가사였고, 그 비용을 요청마다 냈다. 반환 모양은 getAllSongsRuntime과 같되
// stanzas가 빈 배열이다 — 가사가 필요하면 전량 경로나 getSongRuntime을 써야 한다.
export async function getAllSongsMeta() {
  if (!databaseContentEnabled()) return getAllSongs();
  try {
    const rows = await listCachedContentMeta("song");
    return rows
      .map((row) => parseSongRecord(row.slug, row.raw))
      .filter((song) => !song.duplicate_of)
      .sort((a, b) => (b.published || b.date || "").localeCompare(a.published || a.date || ""));
    ...
    noteContentFallback();
    console.error(`[songs] DB 메타 읽기·파싱 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllSongs();
  }
}

// 상세 한 건 — 전곡을 불러 find하지 않는다.
//
// 예전에는 곡 하나를 열 때 928곡을 전부 읽고 파싱한 뒤 그중 하나를 골랐다.
// generateMetadata와 페이지 본체가 각각 부르니 요청당 두 번이었다.
export async function getSongRuntime(slug) {
  if (!databaseContentEnabled()) return getSong(slug);
  try {
    const row = await readCachedContentRow("song", slug);
    return row ? parseSongRecord(slug, row.raw) : null;
  } catch (err) {
    noteContentFallback();
    console.error(`[songs] ${slug} DB 읽기 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getSong(slug);
  }
}

export function getSong(slug) {
  const file = path.join(SONGS_DIR, `${String(slug || "").replace(/[^a-zA-Z0-9가-힣ぁ-んァ-ン一-龯._-]/g, "")}.md`);
  if (!fs.existsSync(file)) return null;
  return parseSongRecord(slug, fs.readFileSync(file, "utf8"));
}
...
export function albumCompanions(song, songs) {
  const album = String(song?.album || "").trim();
  const artist = String(song?.artist || "").trim();
  if (!album || !artist) return [];
  const group = songs.filter((candidate) => String(candidate.album || "").trim() === album && String(candidate.artist || "").trim() === artist);
  return group.length >= 2 ? group.filter((candidate) => candidate.slug !== song.slug).sort((a, b) => Number(a.year || 0) - Number(b.year || 0) || a.title.localeCompare(b.title, "ko")) : [];
}
