...
export async function getAllSongsRuntime() {
  ...
  let rows;
  try {
    rows = await listCachedContentRows("song");
  } catch (err) {
    noteContentFallback();
    console.error(`[songs] DB 읽기 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllSongs();
  }
  return rows
...
export async function getAllSongsMeta() {
  ...
  let rows;
  try {
    rows = await listCachedContentMeta("song");
  } catch (err) {
    noteContentFallback();
    console.error(`[songs] DB 메타 읽기 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllSongs();
  }
  return rows
    .map((row) => parseSongRecord(row.slug, row.raw.replace(/\r\n/g, "\n")))
