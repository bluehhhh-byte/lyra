...
import { databaseContentEnabled, getContentDb, packRows, rowTag, unpackRows } from "./content-db.js";
...
const koreanTag = rowTag("song", "백예린-fuckin-new-year");
assert.match(koreanTag, /^lyra-song-row-[a-f0-9]{64}$/);
assert.equal(koreanTag, rowTag("song", "백예린-fuckin-new-year"));
assert.notEqual(koreanTag, rowTag("song", "다른-곡"));
assert.throws(
  () => validateHeaderValue("x-next-cache-tags", "lyra-song-row-백예린"),
  /Invalid character/,
  "기존 비ASCII 태그는 Node 응답 헤더에서 500을 일으킨다"
);
assert.doesNotThrow(() => validateHeaderValue("x-next-cache-tags", koreanTag));
console.log("content-db cache tag header safety ok");
