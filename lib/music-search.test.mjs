...
// 앨범명으로 짚어 오는 검색 — 제목·아티스트에 없는 낱말만 센다
const onMacabre = { ...apple, trackId: 14, title: "Zan", artist: "DIR EN GREY", album: "MACABRE" };
const onVestige = { ...apple, trackId: 15, title: "Cage", artist: "DIR EN GREY", album: "Vestige of Scratches" };
assert.ok(albumMatchesQuery(onMacabre, "dir en grey macabre"));
assert.ok(!albumMatchesQuery(onVestige, "dir en grey macabre"));
assert.ok(externalSongScore(onMacabre, "dir en grey macabre") > externalSongScore(onVestige, "dir en grey macabre"));
// 싱글은 대개 타이틀곡 이름을 쓴다. 그 이름으로 앨범 일치를 인정하면 어떤 곡을
// 찾든 정규 앨범 대신 싱글이 뽑힌다 — 제목과 겹치는 낱말은 세지 않는다.
assert.ok(!albumMatchesQuery({ title: "Lemon", artist: "Kenshi Yonezu", album: "Lemon - Single" }, "lemon kenshi yonezu"));

// 같은 곡의 여러 판 중 query가 짚은 판을 남긴다
const single = { ...apple, trackId: 16, title: "Myaku", artist: "DIR EN GREY", album: "MYAKU - Single" };
const best = { ...apple, trackId: 17, title: "Myaku", artist: "DIR EN GREY", album: "Vestige of Scratches" };
assert.equal(mergeExternalSongResults([[best, single]], "myaku dir en grey single")[0].album, "MYAKU - Single");
assert.equal(mergeExternalSongResults([[best, single]], "myaku dir en grey")[0].album, "Vestige of Scratches");
