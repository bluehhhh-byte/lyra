...
async function createFromFiles({ cfg, files, commit, fetcher }) {
  // forceNew를 쓰지 않는다 — 같은 파일 집합이면 Vercel이 기존 배포를 돌려주는 것이
  // 정확히 우리가 원하는 dedupe다. 강제 신규는 같은 커밋을 매번 다시 빌드해
  // 무료 티어 빌드 분을 태웠다.
  ...
  const body = {
    ...
    // CLI 업로드 배포에는 VERCEL_GIT_COMMIT_SHA가 안 들어간다 — /api/version이
    // "dev"만 보여 어느 커밋이 서빙 중인지 알 수 없었다. 직접 심는다.
    env: { LYRA_COMMIT_SHA: commit.sha },
    build: { env: { LYRA_COMMIT_SHA: commit.sha } },
    meta: {
