...
export function releaseRecordGaps(songs) {
  ...
  for (const song of songs) {
    const release = Number(song.year);
    const recorded = Number(String(song.published || song.date || "").slice(0, 4));
    if (!Number.isInteger(release) || !Number.isInteger(recorded)) { missing += 1; continue; }
