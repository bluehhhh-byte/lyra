...
import {
  ...
  monthCharacterLabel, INSUFFICIENT_SAMPLE_LABEL, MOOD_MIN_SAMPLE,
} from "./emotion-model.js";

// 축별 작은 변화로 보는 MOVE_THRESHOLD(0.4)의 두 배보다 넉넉한 거리다.
// 두 축의 잡음(√(.4²+.4²)≈.57)을 전환점으로 과장하지 않으면서, 좌표 한 칸을
// 넘는 실제 방향 전환은 잡는다. 빈 달을 건너뛴 이동은 월 대 월 비교가 아니므로 제외.
export const TURNING_POINT_DISTANCE = 1.25;
...
export function monthlyStats(entries) {
  ...
  const stats = months.map((month) => {
    ...
    const songs = items.filter((item) => item.type === "song");
    const movies = items.filter((item) => item.type === "movie");
    // 감정은 곡·영화를 가리지 않는다 — 영화에도 같은 15어휘가 붙는다
    // (브리프 §10 Phase 3: 두 세계 균형). 감정 없는 기록은 tally가 거른다.
    const emotions = tally(items.map((item) => item.emotion));
    const center = emotionCenter(items.map((item) => item.emotion));
    ...
    return {
      ...
      character: monthCharacterLabel(center),
  ...
  for (let i = 1; i < stats.length; i++) {
    ...
    if (prev.center && curr.center) {
      ...
      curr.prev.distance = Math.hypot(curr.prev.dv, curr.prev.da);
      curr.turningPoint = curr.prev.gap === 0 && curr.prev.distance >= TURNING_POINT_DISTANCE;
    }
  }
  return stats;
}
...
export function monthSeasonality(stats, minYears = 3) {
  return Array.from({ length: 12 }, (_, index) => {
    ...
    const type = deferred || !center ? INSUFFICIENT_SAMPLE_LABEL : moodType(center);
    return {
      monthNum,
      years,
      sample,
