...
// 만들지 않는다.
import { EMOTIONS, parseEmotion, emotionValence } from "./keywords.js";
...
// 모델의 15개 어휘를 기준 목록으로 먼저 만들기 때문에 한 번도 쓰이지 않은 감정도
// 0회로 남는다. 사용된 값만 tally하면 모델의 사각지대가 화면에서 사라진다.
export function emotionUsage(emotions) {
  const counts = new Map(EMOTIONS.map((emotion) => [emotion, 0]));
  for (const value of emotions) {
    const emotion = parseEmotion(value);
    if (counts.has(emotion)) counts.set(emotion, counts.get(emotion) + 1);
  }
  return EMOTIONS.map((emotion, order) => [emotion, counts.get(emotion), order])
...
export const INSUFFICIENT_SAMPLE_LABEL = "판단 유보";
...
export function moodType(center, { entropy = 0 } = {}) {
  if (!center || center.n < MOOD_MIN_SAMPLE) return INSUFFICIENT_SAMPLE_LABEL;
  const { v, a } = center;
