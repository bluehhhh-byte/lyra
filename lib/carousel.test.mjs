...
import {
  buildCarousel, splitLines, autoSelect, autoPick,
  hookScore, suggestHooks, CAROUSEL_SLIDES, LYRIC_PARTS,
} from "./carousel.js";
...
// 캡션 후크 — 고른 가사 중 가장 후크다운 줄. 짧고 1인칭인 발화가 위로 온다.
{
  assert.ok(hookScore({ en: "나를 베어도 돼" }) > hookScore({ en: "1999년의 여름밤 매미가 울던" }));
  assert.equal(hookScore({ en: "" }), 0);
  const idx = autoPick(NINE);
  assert.ok(idx >= 0 && NINE[idx]);
  assert.equal(idx, suggestHooks(NINE, 1)[0].index);
}
