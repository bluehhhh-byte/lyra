...
import {
  buildCarousel, splitLines, autoSelect, fitCarouselNoteLayout,
  CAROUSEL_THEME, CAROUSEL_SLIDES, LYRIC_PARTS, MAX_SELECTED_LINES,
} from "./carousel.js";
...
const FIFTEEN = Array.from({ length: 15 }, (_, i) => line(i, `가사 ${i + 1}`));
const NOTE = "자신을 숲이라 했다가 바다가 아니었느냐고 되묻기를 반복한다.";
const APPEARANCE = "〈더 퍼스트 슬램덩크〉 (2022) · 극장판 애니메이션 · 엔딩";
const LOSE_CONTROL_NOTE = "영화 《고질라》(1998)의 삽입곡이자 일본판 사운드트랙 수록곡이다. 끊어진 마음과 검게 물드는 그림자, 죽은 뒤 태어난 본능이라는 가사가 날카롭게 뒤틀리는 밴드 사운드와 맞물려 통제가 무너지는 순간을 괴수의 각성처럼 밀어붙인다.";

// 5장 구성 — 커버 · 곡 설명 · 가사×3
{
  const { slides, error } = buildCarousel({ selected: NINE, note: NOTE, appearance: APPEARANCE });
  assert.equal(error, "");
  ...
  assert.equal(slides[1].appearance, APPEARANCE);
  ...
  assert.deepEqual(slides.slice(2).map((s) => [s.part, s.parts]), [[1, 3], [2, 3], [3, 3]]);
}

// 분배 — 앞 조각이 크거나 같게, 순서 보존
{
  assert.deepEqual(splitLines(FIFTEEN).map((c) => c.length), [5, 5, 5]);
...
// 기본 선택 — 누른 줄부터 스물한 줄, 연 경계를 넘어 이어 담는다
{
  const many = Array.from({ length: 30 }, (_, i) => line(i, `줄 ${i}`));
  assert.deepEqual(autoSelect(many, 0), Array.from({ length: 21 }, (_, i) => i));
  assert.deepEqual(autoSelect(many, 5), Array.from({ length: 21 }, (_, i) => i + 5));
  // 뒤쪽에서 시작해 스물한 줄이 안 되면 앞에서 채운다
  assert.equal(autoSelect(many, 27).length, 21);
  assert.ok(autoSelect(many, 27, 9).includes(29), "끝 줄을 버리지 않는다");
  assert.ok(autoSelect(many, 27).includes(9), "앞에서 채워 스물한 줄을 맞춘다");
}

assert.equal(LYRIC_PARTS, 3);
assert.equal(MAX_SELECTED_LINES, 21);

// Lose Control처럼 긴 설명도 수록 정보가 있는 240px 본문 영역 안에서 전부 보여야 한다.
{
  const layout = fitCarouselNoteLayout({
    maxHeight: 240,
    wrapAtSize: (fontSize) => {
      const charsPerLine = Math.max(1, Math.floor((CAROUSEL_THEME.width - CAROUSEL_THEME.padding * 2) / fontSize));
      return Array.from({ length: Math.ceil(Array.from(LOSE_CONTROL_NOTE).length / charsPerLine) }, (_, index) =>
        LOSE_CONTROL_NOTE.slice(index * charsPerLine, (index + 1) * charsPerLine));
    },
  });
  assert.equal(layout.fits, true, "Lose Control 설명은 수록 정보 위에서 잘리면 안 됩니다");
  assert.equal(layout.lines.join(""), LOSE_CONTROL_NOTE, "맞춤 과정에서 설명 문장을 버리면 안 됩니다");
  assert.ok(layout.height <= 240, "설명의 실제 출력 높이가 가용 높이를 넘어가면 안 됩니다");
}
