...
import { buildCarousel, groupByStanza, hookScore, suggestHooks, stanzaFits, CAROUSEL_SLIDES } from "./carousel.js";
...
// 4장 구성 — 후크 · 여는 연 · 후크가 속한 연 · 노트
{
  ...
  assert.deepEqual(slides.map((s) => s.role), ["hook", "opening", "peak", "note"]);
  ...
  // 4장 — 해설을 원문 자리에 넣는다(대역 없음)
  assert.equal(slides[3].lines[0].en, NOTE);
  assert.equal(slides[3].lines[0].ko, "");
}

// 노트가 없는 곡 — 3장으로 끝난다. 빈 카드를 만들지 않는다.
{
  const { slides } = buildCarousel({ lines, hookIndex: 5, note: "" });
  assert.equal(slides.length, 3);
  assert.ok(!slides.some((s) => s.role === "note"));
}
