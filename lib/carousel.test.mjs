...
// 기본 선택 — 누른 줄부터 스물네 줄, 연 경계를 넘어 이어 담는다
{
  const many = Array.from({ length: 30 }, (_, i) => line(i, `줄 ${i}`));
  assert.deepEqual(autoSelect(many, 0), Array.from({ length: 24 }, (_, i) => i));
  assert.deepEqual(autoSelect(many, 5), Array.from({ length: 24 }, (_, i) => i + 5));
  // 뒤쪽에서 시작해 스물네 줄이 안 되면 앞에서 채운다
  assert.equal(autoSelect(many, 27).length, 24);
  assert.ok(autoSelect(many, 27, 9).includes(29), "끝 줄을 버리지 않는다");
  assert.ok(autoSelect(many, 27).includes(9), "앞에서 채워 스물네 줄을 맞춘다");
}

assert.equal(LYRIC_PARTS, 3);
assert.equal(MAX_SELECTED_LINES, 24);
