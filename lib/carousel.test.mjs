...
// 4장 구성 — 커버 · 후크 · 여는 연 · 후크가 속한 연.
// 커버가 1장인 이유: 피드 썸네일이 곧 1장이라, 앨범 아트가 썸네일이 되면
// 계정 그리드가 앨범 진열장으로 읽힌다.
{
  const { slides, error } = buildCarousel({ lines, hookIndex: 5, note: NOTE });
  assert.equal(error, "");
  assert.equal(slides.length, CAROUSEL_SLIDES);
  assert.deepEqual(slides.map((s) => s.role), ["cover", "hook", "opening", "peak"]);

  // 1장 — 앨범 커버 + 해설. 전용 렌더러가 그리므로 lines가 아니라 note를 싣는다.
  assert.equal(slides[0].note, NOTE);
  assert.deepEqual(slides[0].lines, [], "가사 줄을 그리지 않는다");

  // 2장 — 후크 한 줄, 번역 없이. 뜻은 4장에서 열린다.
  assert.deepEqual(slides[1].lines, [{ en: "날 지나치지 마 날 보아줘", ko: "" }]);

  // 3장 — 후크가 속하지 않은 연
  assert.equal(slides[2].lines.length, 4);
  assert.equal(slides[2].lines[0].en, "난 저기 숲이 돼볼게");
  assert.ok(slides[2].lines[0].ko, "여는 연에는 번역이 붙는다");

  // 4장 — 후크가 속한 연. 2장의 줄이 여기 다시 나오는 것이 설계의 핵심이다.
  assert.equal(slides[3].lines.length, 3);
  assert.ok(
    slides[3].lines.some((l) => l.en === "날 지나치지 마 날 보아줘"),
    "후크는 자기 연 안에서 맥락과 함께 다시 나온다"
  );
}

// 해설이 없는 곡도 커버 장은 남는다 — 앨범·곡 정보만으로 성립하고,
// 계정의 얼굴이 되는 장을 곡 사정으로 빼지 않는다.
{
  const { slides } = buildCarousel({ lines, hookIndex: 5, note: "" });
  assert.equal(slides.length, 4);
  assert.equal(slides[0].role, "cover");
  assert.equal(slides[0].note, "");
}
...
// 첫 연에서 후크를 고르면 — 여는 연은 다른 연이 된다
{
  ...
  assert.equal(slides[2].lines[0].en, "길을 터 보일게 나를 베어도 돼", "후크가 속한 연을 여는 연으로 쓰지 않는다");
  assert.ok(slides[3].lines.some((l) => l.en === "난 저기 숲이 돼볼게"));
}

// 여는 연을 직접 고를 수 있다 — 자동 선택이 늘 맞지는 않는다
{
  const { slides } = buildCarousel({ lines, hookIndex: 0, note: NOTE, openingStanzaIndex: 1 });
  assert.equal(slides[2].lines[0].en, "길을 터 보일게 나를 베어도 돼");
}
