...
// 기본 선택 — 누른 줄부터 열여덟 줄, 연 경계를 넘어 이어 담는다
{
  const many = Array.from({ length: 24 }, (_, i) => line(i, `줄 ${i}`));
  assert.deepEqual(autoSelect(many, 0), Array.from({ length: 18 }, (_, i) => i));
  assert.deepEqual(autoSelect(many, 5), Array.from({ length: 18 }, (_, i) => i + 5));
  // 뒤쪽에서 시작해 열여덟 줄이 안 되면 앞에서 채운다
  assert.equal(autoSelect(many, 21).length, 18);
  assert.ok(autoSelect(many, 21, 9).includes(23), "끝 줄을 버리지 않는다");
  assert.ok(autoSelect(many, 21).includes(6), "앞에서 채워 열여덟 줄을 맞춘다");
}

assert.equal(LYRIC_PARTS, 3);
assert.equal(MAX_SELECTED_LINES, 18);
