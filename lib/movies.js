...
export async function getAllMoviesRuntime() {
  ...
  let rows;
  try {
    rows = await listCachedContentRows("movie");
  } catch (err) {
    noteContentFallback();
    console.error(`[movies] DB 읽기 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllMovies();
  }
  return rows
...
export async function getAllMoviesMeta() {
  ...
  let rows;
  try {
    rows = await listCachedContentMeta("movie");
  } catch (err) {
    noteContentFallback();
    console.error(`[movies] DB 메타 읽기 실패 — 파일 백업으로 서빙한다: ${err.message}`);
    return getAllMovies();
  }
  return rows
    .map((row) => parseMovieRecord(row.slug, row.raw.replace(/\r\n/g, "\n")))
...
export async function getMovieRuntime(slug) {
  ...
  try {
    ...
    return row ? parseMovieRecord(slug, row.raw.replace(/\r\n/g, "\n")) : null;
  } catch (err) {
    noteContentFallback();
