...
// 읽고, 한쪽만 고쳐지는 사고가 난다.
import { normalizeSearchText } from "./lyric-search.js";
...
import { searchableLyricLines } from "./lyric-search.js";

// 색인을 만들 때 한 번만 접어 두면 질의마다 접을 필요가 없다. 원문에 섞인
// 전각 공백·NBSP가 보통 공백으로 친 검색어를 빗나가게 하던 것을 여기서 막는다.
const lower = (...values) => normalizeSearchText(values.flat().filter(Boolean).join(" "));
...
async function build() {
  ...
  return {
    songs: songs.map((song) => {
      const lines = searchableLyricLines(song);
      return {
        slug: song.slug,
    ...
    people: people.map((person) => ({
      ...
      title: person.name,
      subtitle: `${person.worksCount}편`,
      image: person.image,
      meta: lower(person.name, person.searchText),
    })),
