...
export const MAX_SELECTED_LINES = 24;
...
// 24줄이면 8/8/8, 10줄이면 4/3/3, 7줄이면 3/2/2. 줄이 조각 수보다 적으면
...
// 본문이 쓸 수 있는 상자. 머리 여백 250 + 꼬리 110 + 서명·진행점 몫이 360이다.
//
// 한때 마지막 장에 "… 이하 생략"을 한 줄 넣고 그 높이(46)를 여기서 미리 뺐다.
// 표시를 뺐으니 예약도 함께 걷는다 — 안 그리는 것을 위해 자리를 비워 두면
// 마지막 장의 가사만 이유 없이 작아진다.
export const CAROUSEL_BODY_TOP = 250;
export function carouselBodyBox() {
  const height = CAROUSEL_THEME.height - 360;
...
export function buildCarousel({ selected, note = "", appearance = "" }) {
  ...
  const slides = [
    ...
    ...chunks.map((chunk, i) => ({
      ...
      part: i + 1,
      parts: chunks.length,
      // 실제로 마지막이 되는 장에만 붙어야 한다 — slice 전에 판정하면
      // 6장짜리 곡에서 보이지 않는 장이 마지막으로 표시된다.
      isLast: false,
  ...
  if (last?.role === "lyrics") last.isLast = true;
