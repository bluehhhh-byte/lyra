...
// 잘렸다는 사실만 밝힌다. 한때 여기에 "프로필 링크로 오세요"·"저장해 두세요"
// 두 줄과 구분선이 있었는데 뺐다 — 카드가 부탁하는 그림이 된다.
// 남은 건 정보다: 이게 전곡이 아니라는 것.
export const CAROUSEL_TRUNCATED_NOTE = "… 이하 생략";
// 생략 표시가 본문 아래에 요구하는 높이. 본문 레이아웃이 이만큼을 먼저 뺀다 —
// 다 그린 뒤에 얹으면 21줄 만선 곡에서 마지막 줄에 붙는다.
export const CAROUSEL_TRUNCATED_HEIGHT = 46;

export function carouselFooterReserve({ isLast = false, truncated = false } = {}) {
  return isLast && truncated ? CAROUSEL_TRUNCATED_HEIGHT : 0;
}

// 본문이 쓸 수 있는 상자. 머리 여백 250 + 꼬리 110 + 서명·진행점 몫이 360이고,
// 발췌인 마지막 장은 거기서 생략 표시 몫을 더 뺀다.
...
// 생략 표시의 기준선 — 예약한 띠 안, 본문 바닥과 서명 사이. 예약 높이와 그리는
// 자리를 두 파일에 따로 적어 뒀다가 표시가 본문 마지막 줄에 붙은 적이 있다
// (2026-09-12 실측: 본문 1050행 끝, 표시 1051행 시작). 한 곳에서 계산한다.
export function carouselTruncatedNoteY() {
  return carouselBodyBox({ isLast: true, truncated: true }).bottom + 34;
}
